...
internal class DrawingContext : DocumentLayouter
{
	...
	[ThreadStatic]
	private static Dictionary<WPicture, IImage> headerImageCollection;
	...
	private Dictionary<WPicture, IImage> HeaderImageCollection
	{
		get
		{
			if (headerImageCollection == null)
			{
				headerImageCollection = new Dictionary<WPicture, IImage>();
			}
	...
	internal IGraphics GraphicsBmp
	{
		get
		{
			if (m_graphicsBmp == null)
			{
				IBitmap bitmap = CreateBitmap(1, 1);
				m_graphicsBmp = GetGraphicsFromImage(bitmap);
	...
	{
		IBitmap bitmap = CreateBitmap(1, 1);
		Graphics = GetGraphicsFromImage(bitmap);
		bitmap.SetResolution(120f, 120f);
	...
	internal void DrawParagraph(WParagraph paragraph, LayoutedWidget ltWidget)
	{
		...
		if ((paragraphFormat.TextureStyle != 0 || !isEmpty) && !IsLinesInteresectWithFloatingItems(ltWidget, isLineContainer: true))
		{
			...
			{
				Graphics.FillRectangle(CreateSolidBrush(paragraphFormat.BackColor), boundsToDrawParagraphBackGroundColor);
			}
			if (resetTransform)
			{
				ResetTransform();
	...
	internal void DrawTextBox(WTextBox textBox, LayoutedWidget ltWidget)
	{
		...
		else if (textBoxFormat.Rotation != 0f)
		{
			...
			if (num != 0f && (textBoxFormat.AutoFit || textBoxFormat.WrappingMode == WrapMode.None || (textBox.IsShape && textBox.Shape.TextFrame.NoWrap)))
			{
				float width = textBoxFormat.Width;
				float height = textBoxFormat.Height;
				if (textBoxFormat.WrappingMode == WrapMode.None || (textBox.IsShape && textBox.Shape.TextFrame.NoWrap))
				{
		...
		if ((num != 0f || (num == 0f && (flipHorizontal || flipVertical))) && textBoxFormat.TextWrappingStyle != TextWrappingStyle.Tight && textBoxFormat.TextWrappingStyle != TextWrappingStyle.Through)
		{
			...
			{
				Graphics.Transform = GetTransformMatrix(ltWidget.Bounds, num, flipHorizontal, flipVertical);
			}
		}
		...
		if (textBoxFormat.VMLPathPoints == null)
		{
			...
			{
				DrawPictureFill(textBoxFormat.FillEfects.ImageRecord, graphicsPath, ltWidget.Bounds, textBox.IsShape ? textBox.Shape.FillFormat.BlipFormat.Transparency : 0f);
			}
			else
			{
				Graphics.FillRectangle(CreateSolidBrush(color), ltWidget.Bounds);
			}
		}
		else
		{
			graphicsPath = new ShapePath(ltWidget.Bounds, null).GetVMLCustomShapePath(textBoxFormat.VMLPathPoints);
			if (graphicsPath.PointCount > 0)
			{
				...
				{
					Graphics.FillPath(CreateSolidBrush(color), graphicsPath);
				}
			}
		}
		...
		if (!textBoxFormat.NoLine)
		{
			...
			borders.Color = textBoxFormat.LineColor;
			IPen pen = GetPen(borders.Left, isParagraphBorder: false);
			if (textBox.Shape != null)
			{
				pen.LineJoin = GetLineJoin(textBox.Shape.LineFormat.LineJoin);
			}
			if (textBoxFormat.LineStyle == TextBoxLineStyle.Simple && lineDashing != 0)
			{
				pen = GetDashStyle(lineDashing, pen);
				pen.StartCap = DocGen.Drawing.LineCap.Flat;
				pen.EndCap = DocGen.Drawing.LineCap.Flat;
			}
			...
			{
				Graphics.DrawRectangle(pen, ltWidget.Bounds.X, ltWidget.Bounds.Y, ltWidget.Bounds.Width, ltWidget.Bounds.Height);
			}
		}
	...
	private void DrawBarTabStop(WParagraph paragraph, LayoutedWidget ltWidget)
	{
		...
		for (int i = 0; i < tabs.Count; i++)
		{
			...
			{
				PointF pt = new PointF(tabs[i].Position + ltWidget.Bounds.X, ltWidget.Bounds.Y);
				PointF pt2 = new PointF(tabs[i].Position + ltWidget.Bounds.X, ltWidget.Bounds.Bottom + paragraph.ParagraphFormat.AfterSpacing);
				IPen pen = CreatePen(Color.Black);
	...
	internal void DrawList(WParagraph paragraph, LayoutedWidget ltWidget, WListFormat listFormat)
	{
		...
		if (flag && listLevel.PicBullet.GetImage(listLevel.PicBullet.ImageBytes, isImageFromScratch: false) != null)
		{
			...
			byte[] imageBytes = listLevel.PicBullet.ImageBytes;
			if (num4)
			{
				imageBytes = WordDocument.RenderHelper.ConvertTiffToPng(imageBytes);
			}
			Graphics.DrawImage(GetImage(imageBytes), new RectangleF(x, num3, sizeF.Width, sizeF.Height));
	...
	private void DrawCommentMarks(RevisionOptions revisionOptions)
	{
		...
		{
			IPen pen = CreatePen(revisionColor, 0.2f);
			IGraphicsPath graphicsPath = CreateGraphicsPath();
	...
	internal void DrawSeparator(WTextRange txtRange, LayoutedWidget ltWidget)
	{
		...
		IPen pen = CreatePen(color, 0.5f);
		Graphics.DrawLine(pen, new PointF(ltWidget.Bounds.X, ltWidget.Bounds.Y + ltWidget.Bounds.Height / 2f), new PointF(ltWidget.Bounds.Right, ltWidget.Bounds.Y + ltWidget.Bounds.Height / 2f));
	}
	...
	internal void DrawSymbol(WSymbol symbol, LayoutedWidget ltWidget)
	{
		...
		EnsureComplexScript(charFormat);
		...
		else
		{
			...
			{
				font2 = WordDocument.RenderHelper.GetFallbackFont(font2, text, FontScriptType.English, FallbackFonts, FontStreams);
			}
			Graphics.DrawString(text, font2, GetBrush(GetTextColor(symbol.CharacterFormat)), textBounds, stringFormat, FontScriptType.English);
		}
		...
		if (picture != null)
		{
			...
			if (picture.ImageBytes != null)
			{
				...
				else
				{
					...
					image = CreateBitmap().Decode(array);
					if (((double)picture.FillRectangle.LeftOffset != 0.0 || (double)picture.FillRectangle.RightOffset != 0.0 || (double)picture.FillRectangle.TopOffset != 0.0 || (double)picture.FillRectangle.BottomOffset != 0.0) && image is IBitmap && WordDocument.RenderHelper.HasBitmap(image))
					{
						...
						{
							using IImage image2 = (image as IBitmap).Clone(cropRectangle, image.PixelFormat);
							using MemoryStream memoryStream = new MemoryStream();
							image2.Save(memoryStream, image.RawFormat);
							memoryStream.Position = 0L;
							image = CreateImageFromStream(new MemoryStream(memoryStream.ToArray()));
						}
					}
				}
			}
			...
			if (ltwidget.Widget.LayoutInfo.IsVerticalText && ltwidget.Widget is WPicture)
			{
				...
				if (ownerParagraphValue.IsInCell)
				{
					...
					if (ownerCellLayoutedWidget != null)
					{
						...
						{
							Graphics.TranslateTransform(bounds2.X + bounds2.Y + bounds2.Width, bounds2.Y - bounds2.X);
							Graphics.RotateTransform(90f);
						}
						else
						{
							Graphics.TranslateTransform(bounds2.X - bounds2.Y, bounds2.X + bounds2.Y + bounds2.Height);
				...
				else if (ownerEntity is WTextBox)
				{
					...
					float y = top - bounds.Y + (bounds.X - left) + num;
					if (wTextBox.TextBoxFormat.TextDirection == DocGen.DocIO.DLS.TextDirection.VerticalTopToBottom)
					{
						...
						Graphics.TranslateTransform(wTextBox.TextLayoutingBounds.X + wTextBox.TextLayoutingBounds.Y + wTextBox.TextLayoutingBounds.Height - widthToShiftVerticalText, wTextBox.TextLayoutingBounds.Y - wTextBox.TextLayoutingBounds.X);
						Graphics.RotateTransform(90f);
					}
					else
					{
						Graphics.TranslateTransform(bounds.Y - top, y);
					}
				}
				else if (ownerEntity is Shape)
				{
					...
					float y2 = top2 - bounds.Y + (bounds.X - left2) + num2;
					...
					{
						float x = GetCellHeightForVerticalText(picture) + left2 - bounds.Y - bounds.Width - num2;
						Graphics.TranslateTransform(x, y2);
					}
					else
					{
						Graphics.TranslateTransform(bounds.Y - top2, y2);
					}
				}
				else if (ownerParagraphValue.Owner.Owner is ChildShape)
				{
					...
					float y3 = top3 - bounds.Y + (bounds.X - left3) + num3;
					...
					{
						float x2 = GetCellHeightForVerticalText(picture) + left3 - bounds.Y - bounds.Width - num3;
						Graphics.TranslateTransform(x2, y3);
					}
					else
					{
						Graphics.TranslateTransform(bounds.Y - top3, y3);
					}
				}
			}
			float num4 = 0f;
			...
			if (ownerParagraphValue2 != null && ((picture.LayoutInCell && picture.Document.Settings.CompatibilityMode == CompatibilityMode.Word2013) || picture.TextWrappingStyle == TextWrappingStyle.Inline) && (!ownerParagraphValue2.IsInCell || !IsTableInTextBoxOrShape(ownerParagraphValue2.GetOwnerEntity(), checkTextBoxOnly: true)) && (!ownerParagraphValue2.IsInCell || !(ownerParagraphValue2.GetOwnerEntity() is WTableCell) || !(ownerParagraphValue2.GetOwnerEntity() as WTableCell).OwnerRow.OwnerTable.IsInCell || !(bounds.Width < ltwidget.Owner.Owner.Owner.Bounds.Width)))
			{
				...
				if (flag3 && Graphics.ClipBounds.Width == 0f)
				{
					Graphics.ResetTransform();
					return;
				}
			...
			{
				if (image is IBitmap && WordDocument.RenderHelper.HasBitmap(image))
				{
			...
			if ((rotation != 0f || (rotation == 0f && (picture.FlipHorizontal || picture.FlipVertical))) && !ltwidget.Widget.LayoutInfo.IsVerticalText && picture.TextWrappingStyle != TextWrappingStyle.Tight && picture.TextWrappingStyle != TextWrappingStyle.Through)
			{
				...
				Graphics.Transform = GetTransformMatrix(bounds, rotation, picture.FlipHorizontal, picture.FlipVertical);
			}
			...
			else if (ownerEntity3 is WTextBox)
			{
				...
				{
					Graphics.Transform = GetTransformMatrix(m_rotateTransform, wTextBox2.TextBoxFormat.Rotation, wTextBox2.TextBoxFormat.FlipHorizontal, wTextBox2.TextBoxFormat.FlipVertical);
				}
			...
			else if (ownerEntity3 is Shape)
			{
				...
				{
					Graphics.Transform = GetTransformMatrix(m_rotateTransform, shape2.Rotation, shape2.FlipHorizontal, shape2.FlipVertical);
				}
			}
			IImageAttributes imageAttributes = null;
			if (image is IBitmap && picture.FillFormat.BlipFormat.BlipTransparency == BlipTransparency.GrayScale)
			{
				IColorMatrix newColorMatrix = CreateColorMatrix(0f, 1f, 0f);
			...
			else
			{
				...
				{
					Graphics.DrawImage(GetImage(image), new RectangleF(bounds.X + lineWidth2, bounds.Y + lineWidth2, size.Width, size.Height));
				}
				else
				{
					Graphics.DrawImage(GetImage(image), new Rectangle((int)(bounds.X + lineWidth2), (int)(bounds.Y + lineWidth2), (int)size.Width, (int)size.Height), 0f, 0f, image.Width, image.Height, GraphicsUnit.Pixel, imageAttributes);
				}
				if (lineWidth2 > 0f)
				{
					IPen pen = CreatePen(picture.PictureShape);
					pen.Width = lineWidth2;
					Graphics.DrawRectangle(pen, bounds.X + lineWidth2 / 2f, bounds.Y + lineWidth2 / 2f, bounds.Width - lineWidth2, bounds.Height - lineWidth2);
				}
			}
			if (image != null && !flag2)
			{
	...
	private IColorMatrix CreateColorMatrix(float b, float c, float s)
	{
		...
		return CreateColorMatrix(new float[5][]
		{
			new float[5]
			{
				c * (num5 + s),
	...
	private void DrawInlinePictureShape(WPicture picture, RectangleF bounds, SizeF size, IImage image)
	{
		...
		{
			AdjustPictureBrightnessAndContrast(image as IBitmap, picture, ref imageAttributes, isWatermark: false);
		}
		Graphics.DrawImage(GetImage(image), new RectangleF(bounds.X + lineWidth, bounds.Y + lineWidth2, size.Width, size.Height));
		...
		if (lineWidth > 0f)
		{
			pen.Color = color2;
			pen.Width = lineWidth;
			pen = GetPictureBorderPen(picture.PictureShape, picture.PictureShape.PictureDescriptor.BorderLeft, pen);
			Graphics.DrawLine(pen, new PointF(bounds.Left + lineWidth / 2f, bounds.Top), new PointF(bounds.Left + lineWidth / 2f, bounds.Bottom));
		}
		if (lineWidth2 > 0f)
		{
			pen.Color = color4;
			pen.Width = lineWidth2;
			pen = GetPictureBorderPen(picture.PictureShape, picture.PictureShape.PictureDescriptor.BorderTop, pen);
			Graphics.DrawLine(pen, new PointF(bounds.Left, bounds.Top + lineWidth2 / 2f), new PointF(bounds.Right, bounds.Top + lineWidth2 / 2f));
		}
		if (lineWidth3 > 0f)
		{
			pen.Color = color3;
			pen.Width = lineWidth3;
			pen = GetPictureBorderPen(picture.PictureShape, picture.PictureShape.PictureDescriptor.BorderRight, pen);
			Graphics.DrawLine(pen, new PointF(bounds.Right - lineWidth3 / 2f, bounds.Top), new PointF(bounds.Right - lineWidth3 / 2f, bounds.Bottom));
		}
		if (lineWidth4 > 0f)
		{
			pen.Color = color;
			pen.Width = lineWidth4;
			pen = GetPictureBorderPen(picture.PictureShape, picture.PictureShape.PictureDescriptor.BorderBottom, pen);
			Graphics.DrawLine(pen, new PointF(bounds.Left, bounds.Bottom - lineWidth4 / 2f), new PointF(bounds.Right, bounds.Bottom - lineWidth4 / 2f));
		}
	}

	private IPen CreatePen(InlineShapeObject inlineShapeObject)
	{
		Color colorBorder = GetColorBorder(inlineShapeObject);
		IPen pen = CreatePen(colorBorder);
		pen = GetDashAndLineStyle(inlineShapeObject, pen);
		pen.LineJoin = GetLineJoin((DocGen.DocIO.ReaderWriter.DataStreamParser.Escher.LineJoin)inlineShapeObject.ShapeContainer.GetPropertyValue(470));
		pen.DashCap = GetLineCap((DocGen.DocIO.ReaderWriter.DataStreamParser.Escher.LineCap)inlineShapeObject.ShapeContainer.GetPropertyValue(471));
		return pen;
	}
	...
	private DashCap GetLineCap(DocGen.DocIO.ReaderWriter.DataStreamParser.Escher.LineCap lineCap)
	{
		return lineCap switch
		{
			DocGen.DocIO.ReaderWriter.DataStreamParser.Escher.LineCap.Square => DashCap.Triangle, 
			DocGen.DocIO.ReaderWriter.DataStreamParser.Escher.LineCap.Round => DashCap.Round, 
			_ => DashCap.Flat, 
		};
	}

	private DocGen.Drawing.LineJoin GetLineJoin(DocGen.DocIO.ReaderWriter.DataStreamParser.Escher.LineJoin lineJoin)
	{
		return lineJoin switch
		{
			DocGen.DocIO.ReaderWriter.DataStreamParser.Escher.LineJoin.Bevel => DocGen.Drawing.LineJoin.Bevel, 
			DocGen.DocIO.ReaderWriter.DataStreamParser.Escher.LineJoin.Miter => DocGen.Drawing.LineJoin.Miter, 
			_ => DocGen.Drawing.LineJoin.Round, 
		};
	}
	...
	internal void DrawEquationField(FontScriptType scriptType, LayoutedEQFields ltEQField, WCharacterFormat charFormat)
	{
		...
		IBrush brush = GetBrush(textColor);
		StringFormat stringFormat = new StringFormat(StringFormt);
		stringFormat.FormatFlags &= ~StringFormatFlags.DirectionRightToLeft;
		EnsureComplexScript(charFormat);
		for (int i = 0; i < ltEQField.ChildEQFileds.Count; i++)
		{
			if (ltEQField.ChildEQFileds[i] is TextEQField)
			{
				...
				RectangleF rectangle = new RectangleF(textEQField.Bounds.X, textEQField.Bounds.Y + num, textEQField.Bounds.Width, textEQField.Bounds.Height);
				if (FallbackFonts != null && FallbackFonts.Count > 0)
				{
					font = WordDocument.RenderHelper.GetFallbackFont(font, textEQField.Text, scriptType, FallbackFonts, FontStreams);
				}
				Graphics.DrawString(textEQField.Text, font, brush, rectangle, stringFormat, scriptType);
			}
			else if (ltEQField.ChildEQFileds[i] is LineEQField)
			{
				LineEQField lineEQField = ltEQField.ChildEQFileds[i] as LineEQField;
				IPen pen = CreatePen(Color.Black, 0.5f);
				Graphics.DrawLine(pen, lineEQField.Point1, lineEQField.Point2);
			}
	...
	private void DrawRadicalSwitch(FontScriptType scriptType, LayoutedEQFields ltEQField, WCharacterFormat charFormat)
	{
		...
		stringFormat.FormatFlags &= ~StringFormatFlags.DirectionRightToLeft;
		EnsureComplexScript(charFormat);
		IPen pen = CreatePen(Color.Black, 0.7f);
	...
	{
		if (!EnableComplexScript && charFormat.ComplexScript)
		{
	...
	{
		PointF pointF = new PointF(bounds.X + leftBorderWidth, bounds.Y + topBorderWidth);
		PointF pointF2 = new PointF(bounds.X + leftBorderWidth + size.Width, bounds.Y + topBorderWidth);
		PointF pointF3 = new PointF(bounds.X + leftBorderWidth, bounds.Y + topBorderWidth + size.Height);
	...
	internal void DrawString(FontScriptType scriptType, string text, WCharacterFormat charFormat, WParagraphFormat paraFormat, RectangleF bounds, float clipWidth, LayoutedWidget ltWidget)
	{
		...
		{
			ISolidBrush brush2 = GetBrush(charFormat.TextBackgroundColor);
			Graphics.FillRectangle(brush2, x, num2, bounds.Width, num);
		}
		if (!charFormat.HighlightColor.IsEmpty && !hasCommentsHighlighter)
		{
			...
			ISolidBrush brush3 = CreateSolidBrush(hightLightColor);
			Graphics.FillRectangle(brush3, x, num2, bounds.Width, num);
		}
		if (IsListCharacter)
		{
		...
		if (charFormat.Emboss)
		{
			...
			ISolidBrush brush4 = CreateSolidBrush(Color.Gray);
			font2 = ((FallbackFonts == null || FallbackFonts.Count <= 0) ? GetAlternateFontToRender(text, font2, charFormat) : WordDocument.RenderHelper.GetFallbackFont(font2, text, scriptType, FallbackFonts, FontStreams));
			if (IsUnicode(text))
			{
				DrawUnicodeString(text, font2, brush4, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num3, bounds.Width, bounds.Height), stringFormat, scriptType);
			}
			else
			{
				Graphics.DrawString(text, font2, brush4, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num3, bounds.Width, bounds.Height), stringFormat, scriptType);
			}
		}
		if (charFormat.Engrave)
		{
			...
			ISolidBrush brush5 = CreateSolidBrush(Color.Gray);
			font3 = ((FallbackFonts == null || FallbackFonts.Count <= 0) ? GetAlternateFontToRender(text, font3, charFormat) : WordDocument.RenderHelper.GetFallbackFont(font3, text, scriptType, FallbackFonts, FontStreams));
			if (IsUnicode(text))
			{
				DrawUnicodeString(text, font3, brush5, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num3, bounds.Width, bounds.Height), stringFormat, scriptType);
			}
			else
			{
				Graphics.DrawString(text, font3, brush5, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num3, bounds.Width, bounds.Height), stringFormat, scriptType);
			}
		}
			...
			else
			{
				...
				else
				{
					...
					else
					{
						if (FallbackFonts != null && FallbackFonts.Count > 0)
						{
							font4 = WordDocument.RenderHelper.GetFallbackFont(font4, text, scriptType, FallbackFonts, FontStreams);
						}
						Graphics.DrawString(text, font4, brush, new RectangleF(bounds.X, bounds.Y + num3, bounds.Width, bounds.Height), stringFormat, scriptType);
					}
		...
		if ((ExportBookmarks & ExportBookmarkType.Headings) != 0 && (currParagraph != null || paraFormat != null) && !currParagraph.IsInCell)
		{
			...
			if (headingLevel >= 0 && headingLevel < 9 && !currParagraph.OmitHeadingStyles() && currParagraphIndex != currParagraph.Index)
			{
				...
				DocumentLayouter.Bookmarks.Add(new BookmarkPosition(bookmarkName, DocumentLayouter.PageNumber, bounds, headingLevel));
	...
	private void CheckPreOrNextSiblingIsTab(ref WCharacterFormat charFormat, ref RectangleF textBounds, LayoutedWidget ltWidget, ref bool isSameLine)
	{
		...
		for (int i = 0; i < owner.ChildWidgets.Count; i++)
		{
			...
			if (!(layoutedWidget.Widget.LayoutInfo is WTextRange.LayoutTabInfo) || charFormat.UnderlineStyle == UnderlineStyle.None || (!IsSame(layoutedWidget.Bounds.X, textBounds.Right, 1) && !IsSame(layoutedWidget.Bounds.Right, textBounds.X, 1)))
			{
				continue;
			}
	...
	private void DrawSmallCapString(FontScriptType scriptType, CharacterRangeType characterRangeType, string text, WCharacterFormat charFormat, RectangleF bounds, DocGen.Drawing.Font font, StringFormat format, IBrush textBrush, bool isCharacterSpacing)
	{
		...
		for (int j = 0; j < list.Count; j++)
		{
			if (list[j] == 'c')
			{
				...
				else
				{
					if (FallbackFonts != null && FallbackFonts.Count > 0)
					{
						font3 = WordDocument.RenderHelper.GetFallbackFont(font3, list2[j], scriptType, FallbackFonts, FontStreams);
					}
					Graphics.DrawString(list2[j], font3, textBrush, new RectangleF(bounds.X + num, bounds.Y + num2, bounds.Width, bounds.Height), format, scriptType);
				}
				num += empty.Width;
				continue;
			}
			...
			else
			{
				if (FallbackFonts != null && FallbackFonts.Count > 0)
				{
					font4 = WordDocument.RenderHelper.GetFallbackFont(font4, list2[j], scriptType, FallbackFonts, FontStreams);
				}
				Graphics.DrawString(list2[j], font4, textBrush, new RectangleF(bounds.X + num, bounds.Y + (ascent - ascent2) + num2, bounds.Width, bounds.Height), format, scriptType);
			}
			num += empty.Width;
		}
	}
	...
	internal void DrawStringBasedOnCharSpacing(FontScriptType scriptType, CharacterRangeType characterRangeType, DocGen.Drawing.Font font, IBrush textBrush, RectangleF bounds, string text, StringFormat format, WCharacterFormat charFormat)
	{
		...
		for (int i = 0; i < text2.Length; i++)
		{
			...
			if (FallbackFonts != null && FallbackFonts.Count > 0)
			{
				font = WordDocument.RenderHelper.GetFallbackFont(font, c.ToString(), scriptType, FallbackFonts, FontStreams);
			}
			if (bounds.X <= bounds.X + num4)
			{
				Graphics.DrawString(c.ToString(), font, textBrush, new RectangleF(bounds.X + num4, bounds.Y + num3, width, bounds.Height), format, scriptType);
				if (num2 > 0f)
				{
					Graphics.DrawString(" ", font, textBrush, new RectangleF(bounds.X + num4 + width, bounds.Y + num3, num2, bounds.Height), format, scriptType);
				}
			}
			else
			{
				Graphics.DrawString(c.ToString(), font, textBrush, new RectangleF(bounds.X, bounds.Y + num3, width, bounds.Height), format, scriptType);
			}
			num4 += width + num2;
		}
	}
	...
	private void TransformGraphicsPosition(LayoutedWidget ltWidget, bool isNeedToScale, ref bool isRotateTransformApplied, ref PointF translatePoints, ref float rotationAngle, WParagraph ownerParagraph)
	{
		...
		if (ownerParagraph.IsInCell)
		{
			...
			if (wTableCell.CellFormat.TextDirection == DocGen.DocIO.DLS.TextDirection.VerticalTopToBottom)
			{
				...
				{
					isRotateTransformApplied = true;
					Graphics.TranslateTransform(translatePoints.X, translatePoints.Y);
		...
		else if (ownerEntity is WTextBox)
		{
			...
			if (wTextBox.TextBoxFormat.TextDirection == DocGen.DocIO.DLS.TextDirection.VerticalTopToBottom || wTextBox.TextBoxFormat.TextDirection == DocGen.DocIO.DLS.TextDirection.VerticalFarEast)
			{
				...
				{
					isRotateTransformApplied = true;
					Graphics.TranslateTransform(xPosition + yPosition + wTextBox.TextLayoutingBounds.Height - widthToShiftVerticalText, yPosition - xPosition);
					Graphics.RotateTransform(90f);
				}
			}
			else
			{
				...
				{
					isRotateTransformApplied = true;
					Graphics.TranslateTransform(xPosition - yPosition + widthToShiftVerticalText, xPosition + yPosition + wTextBox.TextLayoutingBounds.Width);
					Graphics.RotateTransform(270f);
				}
			}
		}
		else if (ownerEntity is Shape)
		{
			...
			if (shape.TextFrame.TextDirection == DocGen.DocIO.DLS.TextDirection.VerticalTopToBottom || shape.TextFrame.TextDirection == DocGen.DocIO.DLS.TextDirection.VerticalFarEast)
			{
				...
				{
					isRotateTransformApplied = true;
					Graphics.TranslateTransform(xPosition2 + yPosition2 + shape.TextLayoutingBounds.Height, yPosition2 - xPosition2);
					Graphics.RotateTransform(90f);
				}
			}
			else
			{
				...
				{
					isRotateTransformApplied = true;
					Graphics.TranslateTransform(xPosition2 - yPosition2, shape.TextLayoutingBounds.Width + xPosition2 + yPosition2);
					Graphics.RotateTransform(270f);
				}
			}
		}
		else
		{
			...
			if (childShape.TextFrame.TextDirection == DocGen.DocIO.DLS.TextDirection.VerticalTopToBottom || childShape.TextFrame.TextDirection == DocGen.DocIO.DLS.TextDirection.VerticalFarEast)
			{
				...
				{
					isRotateTransformApplied = true;
					Graphics.TranslateTransform(xPosition3 + yPosition3 + childShape.TextLayoutingBounds.Height, yPosition3 - xPosition3);
					Graphics.RotateTransform(90f);
				}
			}
			else
			{
				...
				{
					isRotateTransformApplied = true;
					Graphics.TranslateTransform(xPosition3 - yPosition3, childShape.TextLayoutingBounds.Width + xPosition3 + yPosition3);
					Graphics.RotateTransform(270f);
				}
			}
		}
	...
	private void DrawRTLText(FontScriptType scriptType, CharacterRangeType characterRangeType, string text, WCharacterFormat charFormat, DocGen.Drawing.Font font, IBrush textBrush, RectangleF bounds, StringFormat format)
	{
		if (charFormat.CharacterSpacing != 0f)
		{
		...
		else if (!charFormat.Bidi)
		{
			...
			font4 = ((FallbackFonts == null || FallbackFonts.Count <= 0) ? GetAlternateFontToRender(text, font4, charFormat) : WordDocument.RenderHelper.GetFallbackFont(font4, text, scriptType, FallbackFonts, FontStreams));
			Graphics.DrawString(text, font4, textBrush, new RectangleF(bounds.X, bounds.Y + num, bounds.Width, bounds.Height), format, scriptType);
		}
		else
		{
	...
	private void DrawChineseText(FontScriptType scriptType, CharacterRangeType characterRangeType, string text, WCharacterFormat charFormat, DocGen.Drawing.Font font, IBrush textBrush, RectangleF bounds, StringFormat format)
	{
		text = text.Replace('\u200d'.ToString(), "");
		if (text.Length == 0)
		{
		...
		if (font.Name != font3.Name && IsUnicodeText(text))
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				if (!IsUnicodeText(array[i].ToString()))
				{
					...
					else
					{
						...
						font4 = ((FallbackFonts == null || FallbackFonts.Count <= 0) ? GetAlternateFontToRender(text3, font4, charFormat) : WordDocument.RenderHelper.GetFallbackFont(font4, text3, scriptType, FallbackFonts, FontStreams));
						DrawUnicodeString(text3, font4, textBrush, new RectangleF(bounds.X, bounds.Y - num4 + num, bounds.Width, bounds.Height), format, scriptType);
					}
					bounds.X += num2;
					text3 = null;
				}
			...
			if (text3 != null)
			{
				...
				font5 = ((FallbackFonts == null || FallbackFonts.Count <= 0) ? GetAlternateFontToRender(text3, font5, charFormat) : WordDocument.RenderHelper.GetFallbackFont(font5, text3, scriptType, FallbackFonts, FontStreams));
				DrawUnicodeString(text3, font5, textBrush, new RectangleF(bounds.X, bounds.Y - num4 + num, bounds.Width, bounds.Height), format, scriptType);
			}
			else if (text2 != null)
			{
				DrawUnicodeText(scriptType, characterRangeType, text2, charFormat, font, textBrush, new RectangleF(bounds.X, bounds.Y + num3, bounds.Width, bounds.Height), format);
	...
	private void DrawUnicodeText(FontScriptType scriptType, CharacterRangeType characterRangeType, string text, WCharacterFormat charFormat, DocGen.Drawing.Font font, IBrush textBrush, RectangleF bounds, StringFormat format)
	{
		char[] array = text.ToCharArray();
		string text2 = null;
		string text3 = null;
		...
		for (int i = 0; i < array.Length; i++)
		{
			if (array[i] < 'ÿ')
			{
				...
				else
				{
					...
					font3 = ((FallbackFonts == null || FallbackFonts.Count <= 0) ? GetAlternateFontToRender(text3, font3, charFormat) : WordDocument.RenderHelper.GetFallbackFont(font3, text3, scriptType, FallbackFonts, FontStreams));
					DrawUnicodeString(text3, font3, textBrush, new RectangleF(bounds.X, bounds.Y + num2, bounds.Width, bounds.Height), format, scriptType);
				}
				bounds.X += num;
				text3 = null;
			}
		...
		if (text3 != null)
		{
			...
			font4 = ((FallbackFonts == null || FallbackFonts.Count <= 0) ? GetAlternateFontToRender(text3, font4, charFormat) : WordDocument.RenderHelper.GetFallbackFont(font4, text3, scriptType, FallbackFonts, FontStreams));
			DrawUnicodeString(text3, font4, textBrush, new RectangleF(bounds.X, bounds.Y + num2, bounds.Width, bounds.Height), format, scriptType);
		}
		else if (text2 != null)
		{
			num = MeasureString(text2, font, format, charFormat, isMeasureFromTabList: false, isMeasureFromSmallCapString: true, scriptType).Width;
	...
	internal void DrawUnicodeString(FontScriptType scriptType, CharacterRangeType characterRangeType, string text, WCharacterFormat charFormat, DocGen.Drawing.Font font, IBrush textBrush, RectangleF bounds, StringFormat format)
	{
		float num = 0f;
		string fontNameToRender = charFormat.GetFontNameToRender(scriptType);
		if (!IsListCharacter && (fontNameToRender == "DIN Offc" || fontNameToRender == "DIN OT") && currParagraph != null && currParagraph.IsContainDinOffcFont())
		...
		RectangleF rectangle = new RectangleF(bounds.X, bounds.Y + num, bounds.Width, bounds.Height);
		if (FallbackFonts != null && FallbackFonts.Count > 0)
		{
			font2 = WordDocument.RenderHelper.GetFallbackFont(font2, text, scriptType, FallbackFonts, FontStreams);
		}
		Graphics.DrawString(text, font2, textBrush, rectangle, format, scriptType);
	}
	...
	internal void DrawJustifiedLine(WTextRange txtRange, string text, WCharacterFormat charFormat, WParagraphFormat paraFormat, RectangleF bounds, LayoutedWidget ltWidget)
	{
		...
		{
			ISolidBrush brush2 = GetBrush(charFormat.TextBackgroundColor);
			Graphics.FillRectangle(brush2, bounds.X, bounds.Y, bounds.Width, bounds.Height);
		}
		...
		if (!charFormat.HighlightColor.IsEmpty)
		{
			...
			Graphics.FillRectangle(brush3, bounds.X, bounds.Y, bounds.Width, sizeF3.Height);
		}
		ResetTransform();
		float clipWidth = ltWidget.Bounds.Width;
		float clipTopPosition = GetClipTopPosition(bounds, isInlinePicture: false);
		...
		if (charFormat.Emboss)
		{
			...
			if (IsUnicode(text))
			{
				DrawUnicodeString(text, font3, brush4, new RectangleF(bounds.X + 0.2f, bounds.Y + 0.2f + num2, bounds.Width, bounds.Height), stringFormat, scriptType);
			}
			else
			{
				Graphics.DrawString(text, font3, brush4, new RectangleF(bounds.X + 0.2f, bounds.Y + 0.2f + num2, bounds.Width, bounds.Height), stringFormat, scriptType);
			}
		...
		if (charFormat.Engrave)
		{
			...
			if (IsUnicode(text))
			{
				DrawUnicodeString(text, font4, brush5, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num2, bounds.Width, bounds.Height), stringFormat, scriptType);
			}
			else
			{
				Graphics.DrawString(text, font4, brush5, new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num2, bounds.Width, bounds.Height), stringFormat, scriptType);
			}
			textBounds = new RectangleF(bounds.X - 0.2f, bounds.Y - 0.2f + num2, bounds.Width, bounds.Height);
		}
		if (charFormat.AllCaps)
		...
		for (int j = 0; j < array.Length; j++)
		{
			...
			else
			{
				...
				SetClip(clipBounds);
				if (Graphics.ClipBounds.Width == 0f)
				{
					Graphics.ResetTransform();
					ResetClip();
					continue;
				...
				else
				{
					...
					else
					{
						if (FallbackFonts != null && FallbackFonts.Count > 0)
						{
							font5 = WordDocument.RenderHelper.GetFallbackFont(font5, array[j], scriptType, FallbackFonts, FontStreams);
						}
						Graphics.DrawString(array[j], font5, brush, new RectangleF(bounds.X, bounds.Y + num2, num7, bounds.Height), stringFormat2, scriptType);
					}
				}
			}
			...
			if (font6.Underline && j != array.Length - 1)
			{
				...
				float width2 = MeasureString(text2, font6, stringFormat, FontScriptType.English).Width;
				...
				Graphics.DrawString(text2, font7, brush, new RectangleF(bounds.X + num7, bounds.Y + num2, width2, bounds.Height), stringFormat2, scriptType);
			}
			if (!ltWidget.IsLastLine && j != array.Length - 1)
			{
				string text3 = " ";
				RectangleF rectangle = new RectangleF(bounds.X + num7, bounds.Y + num2, num6 + ltWidget.WordSpace, bounds.Height);
				...
				if ((ltWidget.CharacterRange & CharacterRangeType.RTL) == CharacterRangeType.RTL)
				{
					rectangle = new RectangleF(num9 - (num6 + ltWidget.WordSpace), bounds.Y + num2, num6 + ltWidget.WordSpace, bounds.Height);
					num9 -= ltWidget.WordSpace / num8 + num6;
				}
				Graphics.DrawString(text3, font8, brush, rectangle);
	...
	internal void DrawRevisionMark(PointF start, PointF end, Color lineColor, float lineWidth)
	{
		...
		Graphics.ResetClip();
		IPen pen = CreatePen(lineColor, lineWidth);
	...
	private void DrawBorder(Border border, PointF start, PointF end)
	{
		...
		{
			IPen pen = GetPen(border, isParagraphBorder: false);
			Graphics.DrawLine(pen, start, end);
		}
	}
	...
	private void DrawParagraphBorder(Border border, PointF start, PointF end)
	{
		if (border.BorderType != BorderStyle.Cleared && ((border.BorderType == BorderStyle.None && !border.HasNoneStyle) || border.BorderType != 0))
		{
			IPen pen = GetPen(border, isParagraphBorder: true);
			Graphics.DrawLine(pen, start, end);
		}
	}
	...
	{
		IPen pen = GetPen(border.BorderType, border.RenderingLineWidth, border.BorderColor);
		Graphics.DrawLine(pen, start, end);
	}
	...
	internal virtual void DrawTable(WTable table, LayoutedWidget ltWidget)
	{
		if (table.TableFormat.CellSpacing > 0f)
		{
			...
			{
				Graphics.FillRectangle(CreateSolidBrush(table.TableFormat.BackColor), ltWidget.Bounds);
			}
	...
	internal virtual void DrawTableCell(WTableCell cell, LayoutedWidget ltWidget)
	{
		...
		else if (!cell.CellFormat.BackColor.IsEmpty && cell.Owner is WTableRow)
		{
			...
			{
				Graphics.FillRectangle(CreateSolidBrush(cell.CellFormat.BackColor), bounds.X, bounds.Y, bounds.Width, bounds.Height);
			}
		}
		if (flag)
		{
	...
	private void FillCellColor(LayoutedWidget ltWidget)
	{
		...
		foreach (LayoutedWidget item in childWidgets)
		{
			...
			if (item.Bounds.X < ltWidget.Bounds.Right && item.Bounds.Right > ltWidget.Bounds.X)
			{
				...
				{
					Graphics.FillRectangle(CreateSolidBrush((ltWidget.Widget as WTableCell).CellFormat.BackColor), bounds);
				}
			}
		}
	}
	...
	internal void DrawTextureStyle(TextureStyle textureStyle, Color foreColor, Color backColor, RectangleF bounds)
	{
		...
		if (textureStyle.ToString().Contains("Percent"))
		{
			...
			Graphics.FillRectangle(GetBrush(foreColor2), bounds);
		}
	...
	private void FillTexture(TextureStyle textureStyle, Color foreColor, Color backColor, RectangleF bounds)
	{
		...
		case TextureStyle.TextureSolid:
			Graphics.FillRectangle(CreateSolidBrush(foreColor), bounds.X, bounds.Y, bounds.Width, bounds.Height);
			break;
		case TextureStyle.TextureCross:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.Cross, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureDarkDiagonalCross:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.ForwardDiagonal, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureDarkDiagonalDown:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.DarkDownwardDiagonal, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureDarkDiagonalUp:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.DarkUpwardDiagonal, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureDarkHorizontal:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.Horizontal, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureDarkVertical:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.DarkVertical, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureDiagonalCross:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.ForwardDiagonal, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureDiagonalDown:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.LightDownwardDiagonal, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureDiagonalUp:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.LightUpwardDiagonal, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureHorizontal:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.Horizontal, foreColor, backColor), bounds);
			break;
		case TextureStyle.TextureVertical:
			Graphics.FillRectangle(CreateHatchBrush(HatchStyle.Vertical, foreColor, backColor), bounds);
			break;
		}
	}
	...
	private void DrawDoubleLineLeftBorder(CellLayoutInfo cellLayoutInfo, CellLayoutInfo.CellBorder leftBorder, PointF start, PointF end, bool isFirstRow, bool isLastRow, bool isFirstCell, bool isLastCell)
	{
		...
		IPen pen = GetPen(leftBorder.BorderType, borderLineWidthArray[0], leftBorder.BorderColor);
		PointF pt = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y);
		PointF pt2 = new PointF(end.X + borderLineWidthArray[0] / 2f, end.Y);
		...
		{
			pt = new PointF(pt.X, pt.Y + borderLineWidthArray[0] + borderLineWidthArray[1]);
		}
		...
		{
			pt2 = new PointF(pt2.X, pt2.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
		}
		Graphics.DrawLine(pen, pt, pt2);
		PointF pt3 = new PointF(start.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, start.Y);
		PointF pt4 = new PointF(end.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, end.Y);
		IPen pen2 = GetPen(leftBorder.BorderType, borderLineWidthArray[2], leftBorder.BorderColor);
		...
		{
			pt3 = new PointF(pt3.X, pt3.Y + borderLineWidthArray[0] + borderLineWidthArray[1]);
		}
		Graphics.DrawLine(pen2, pt3, pt4);
	}
	...
	private void DrawDoubleLineRightBorder(CellLayoutInfo cellLayoutInfo, CellLayoutInfo.CellBorder rightBorder, PointF start, PointF end, bool isFirstRow, bool isLastRow, bool isFirstCell, bool isLastCell)
	{
		...
		IPen pen = GetPen(rightBorder.BorderType, borderLineWidthArray[0], rightBorder.BorderColor);
		...
		PointF pt3 = new PointF(start.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, start.Y);
		PointF pt4 = new PointF(end.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, end.Y);
		IPen pen2 = GetPen(rightBorder.BorderType, borderLineWidthArray[2], rightBorder.BorderColor);
		...
		{
			pt4 = new PointF(pt4.X, pt4.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
		}
		Graphics.DrawLine(pen2, pt3, pt4);
	}
	...
	private void DrawDoubleLineBottomBorder(CellLayoutInfo cellLayoutInfo, PointF start, PointF end, bool isBiDiTable)
	{
		...
		IPen pen = GetPen(cellLayoutInfo.BottomBorder.BorderType, borderLineWidthArray[0], cellLayoutInfo.BottomBorder.BorderColor);
		PointF pt = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
		...
		{
			pt = new PointF(pt.X + borderLineWidthArray[0] + borderLineWidthArray[1], pt.Y);
		}
		...
		{
			pt2 = new PointF(pt2.X + borderLineWidthArray[0], pt2.Y);
		}
		else if (isBiDiTable && cellLayoutInfo.LeftBorder != null && cellLayoutInfo.LeftBorder.BorderType == cellLayoutInfo.BottomBorder.BorderType && cellLayoutInfo.LeftBorder.RenderingLineWidth == cellLayoutInfo.BottomBorder.RenderingLineWidth)
		{
			pt2 = new PointF(pt2.X + borderLineWidthArray[0], pt2.Y);
		}
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(cellLayoutInfo.BottomBorder.BorderType, borderLineWidthArray[2], cellLayoutInfo.BottomBorder.BorderColor);
		PointF pt3 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		PointF pt4 = new PointF(end.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		if (!isBiDiTable && cellLayoutInfo.RightBorder != null && cellLayoutInfo.RightBorder.BorderType == cellLayoutInfo.BottomBorder.BorderType && cellLayoutInfo.RightBorder.RenderingLineWidth == cellLayoutInfo.BottomBorder.RenderingLineWidth)
		{
			pt4 = new PointF(pt4.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2], pt4.Y);
		}
	...
	private void DrawDoubleLineTopBorder(CellLayoutInfo cellLayoutInfo, CellLayoutInfo.CellBorder topBorder, PointF start, PointF end, bool isStart, bool isEnd)
	{
		...
		IPen pen = GetPen(topBorder.BorderType, borderLineWidthArray[0], topBorder.BorderColor);
		PointF pt = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
		PointF pt2 = new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f);
		...
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(topBorder.BorderType, borderLineWidthArray[2], topBorder.BorderColor);
		PointF pt3 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		PointF pt4 = new PointF(end.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		...
		{
			pt3 = new PointF(pt3.X + borderLineWidthArray[0] + borderLineWidthArray[1], pt3.Y);
		}
		...
		{
			pt4 = new PointF(pt4.X - (borderLineWidthArray[1] + borderLineWidthArray[2]), pt4.Y);
		}
		Graphics.DrawLine(pen2, pt3, pt4);
	}
	...
	private void DrawDoubleLineLeftBorder(Border leftBorder, PointF start, PointF end, bool isTopBorderSame, bool isBetweenBorderSame, bool isBottomBorderSame)
	{
		...
		IPen pen = GetPen(leftBorder.BorderType, borderLineWidthArray[0], leftBorder.Color);
		PointF pt = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y);
		PointF pt2 = new PointF(end.X + borderLineWidthArray[0] / 2f, end.Y);
		...
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(leftBorder.BorderType, borderLineWidthArray[2], leftBorder.Color);
		PointF pt3 = new PointF(start.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, start.Y);
		PointF pt4 = new PointF(end.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, end.Y);
	...
	private void DrawTripleLineLeftBorder(Border leftBorder, PointF start, PointF end, bool isTopBorderSame, bool isBetweenBorderSame, bool isBottomBorderSame)
	{
		...
		if (isBottomBorderSame)
		{
			pt2 = new PointF(pt2.X, pt2.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]);
		}
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(leftBorder.BorderType, borderLineWidthArray[2], leftBorder.Color);
		PointF pt3 = new PointF(start.X + (borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f), start.Y);
		...
		Graphics.DrawLine(pen2, pt3, pt4);
		IPen pen3 = GetPen(leftBorder.BorderType, borderLineWidthArray[4], leftBorder.Color);
		PointF pt5 = new PointF(start.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f, start.Y);
		PointF pt6 = new PointF(end.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f, end.Y);
		if (isTopBorderSame || isBetweenBorderSame)
		{
			pt5 = new PointF(pt5.X, pt5.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3]);
		}
		Graphics.DrawLine(pen3, pt5, pt6);
	}
	...
	private void DrawDoubleLineRightBorder(Border rightBorder, PointF start, PointF end, bool isTopBorderSame, bool isBetweenBorderSame, bool isBottomBorderSame)
	{
		...
		IPen pen = GetPen(rightBorder.BorderType, borderLineWidthArray[0], rightBorder.Color);
		PointF pt = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y);
		PointF pt2 = new PointF(end.X + borderLineWidthArray[0] / 2f, end.Y);
		if (isTopBorderSame || isBetweenBorderSame)
		{
		...
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(rightBorder.BorderType, borderLineWidthArray[2], rightBorder.Color);
		PointF pt3 = new PointF(start.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, start.Y);
		PointF pt4 = new PointF(end.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, end.Y);
		if (isBottomBorderSame)
		{
	...
	private void DrawTripleLineRightBorder(Border rightBorder, PointF start, PointF end, bool isTopBorderSame, bool isBetweenBorderSame, bool isBottomBorderSame)
	{
		...
		if (isTopBorderSame || isBetweenBorderSame)
		{
			pt = new PointF(pt.X, pt.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3]);
		}
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(rightBorder.BorderType, borderLineWidthArray[2], rightBorder.Color);
		PointF pt3 = new PointF(start.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, start.Y);
		...
		Graphics.DrawLine(pen2, pt3, pt4);
		IPen pen3 = GetPen(rightBorder.BorderType, borderLineWidthArray[4], rightBorder.Color);
		PointF pt5 = new PointF(start.X + borderLineWidthArray[0] + borderLineWidthArray[1] + (borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f), start.Y);
		PointF pt6 = new PointF(end.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f, end.Y);
		if (isBottomBorderSame)
		{
			pt6 = new PointF(pt6.X, pt6.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]);
		}
		Graphics.DrawLine(pen3, pt5, pt6);
	}
	...
	private void DrawDoubleLineTopBorder(Border topBorder, PointF start, PointF end, bool isLeftBorderSame, bool isRightBorderSame)
	{
		...
		IPen pen = GetPen(topBorder.BorderType, borderLineWidthArray[0], topBorder.Color);
		PointF pt = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
		PointF pt2 = new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f);
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(topBorder.BorderType, borderLineWidthArray[2], topBorder.Color);
		PointF pt3 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		PointF pt4 = new PointF(end.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		if (isLeftBorderSame)
		{
	...
	private void DrawTripleLineTopBorder(Border topBorder, PointF start, PointF end, bool isLeftBorderSame, bool isRightBorderSame)
	{
		...
		if (isRightBorderSame)
		{
			pt4 = new PointF(pt4.X - (borderLineWidthArray[3] + borderLineWidthArray[4]), pt4.Y);
		}
		Graphics.DrawLine(pen2, pt3, pt4);
		IPen pen3 = GetPen(topBorder.BorderType, borderLineWidthArray[4], topBorder.Color);
		PointF pt5 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f);
		PointF pt6 = new PointF(end.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f);
		if (isLeftBorderSame)
		{
			pt5 = new PointF(pt5.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3], pt5.Y);
		}
		if (isRightBorderSame)
		{
			pt6 = new PointF(pt6.X - (borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]), pt6.Y);
		}
		Graphics.DrawLine(pen3, pt5, pt6);
	}
	...
	private void DrawDoubleLineBottomBorder(Border bottomBorder, PointF start, PointF end, bool isLeftBorderSame, bool isRightBorderSame)
	{
		...
		IPen pen = GetPen(bottomBorder.BorderType, borderLineWidthArray[0], bottomBorder.Color);
		PointF pt = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
		PointF pt2 = new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f);
		if (isLeftBorderSame)
		{
		...
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(bottomBorder.BorderType, borderLineWidthArray[2], bottomBorder.Color);
		PointF pt3 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		PointF pt4 = new PointF(end.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		Graphics.DrawLine(pen2, pt3, pt4);
	}
	...
	private void DrawDoubleLine(WCharacterFormat charFormat, BorderStyle borderType, float lineWidth, PointF start, PointF end)
	{
		...
		IPen pen = GetPen(borderType, borderLineWidthArray[0], borderColor);
		PointF pt = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
		PointF pt2 = new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f);
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(borderType, borderLineWidthArray[2], borderColor);
		PointF pt3 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		PointF pt4 = new PointF(end.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		Graphics.DrawLine(pen2, pt3, pt4);
	}
	...
	private void DrawTripleLineBottomBorder(Border bottomBorder, PointF start, PointF end, bool isLeftBorderSame, bool isRightBorderSame)
	{
		...
		if (isLeftBorderSame)
		{
			pt = new PointF(pt.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3], pt.Y);
		}
		if (isRightBorderSame)
		{
			pt2 = new PointF(pt2.X - (borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]), pt2.Y);
		}
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(bottomBorder.BorderType, borderLineWidthArray[2], bottomBorder.Color);
		PointF pt3 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		...
		Graphics.DrawLine(pen2, pt3, pt4);
		IPen pen3 = GetPen(bottomBorder.BorderType, borderLineWidthArray[4], bottomBorder.Color);
		PointF pt5 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f);
		PointF pt6 = new PointF(end.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f);
		Graphics.DrawLine(pen3, pt5, pt6);
	}
	...
	private void DrawDoubleLineBetweenBorder(Border betweenBorder, PointF start, PointF end, bool isLeftBorderSame, bool isRightBorderSame, Border leftBorder, Border rightBorder, bool isOverlapLeft, bool isOverlapRight)
	{
		...
		IPen pen = GetPen(betweenBorder.BorderType, borderLineWidthArray[0], betweenBorder.Color);
		PointF pt = new PointF(start.X, start.Y + borderLineWidthArray[0] / 2f);
		PointF pt2 = new PointF(end.X, end.Y + borderLineWidthArray[0] / 2f);
		if (isLeftBorderSame)
		{
		...
		{
			pt = new PointF(pt.X + GetLeftRightLineWidht(array, isLeft: true), pt.Y);
		}
		if (isRightBorderSame)
		{
			pt2 = new PointF(pt2.X - (borderLineWidthArray[1] + borderLineWidthArray[2]), pt2.Y);
		...
		{
			pt2 = new PointF(pt2.X - GetLeftRightLineWidht(array2, isLeft: false), pt2.Y);
		}
		Graphics.DrawLine(pen, pt, pt2);
		IPen pen2 = GetPen(betweenBorder.BorderType, borderLineWidthArray[2], betweenBorder.Color);
		PointF pt3 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		PointF pt4 = new PointF(end.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f);
		if (isLeftBorderSame)
		{
		...
		{
			pt3 = new PointF(pt3.X + GetLeftRightLineWidht(array, isLeft: true), pt3.Y);
		}
		if (isRightBorderSame)
		{
			pt4 = new PointF(pt4.X - (borderLineWidthArray[1] + borderLineWidthArray[2]), pt4.Y);
		...
		{
			pt4 = new PointF(pt4.X - GetLeftRightLineWidht(array2, isLeft: false), pt4.Y);
		}
		Graphics.DrawLine(pen2, pt3, pt4);
		if (isOverlapRight || isOverlapLeft)
		{
			if (isOverlapLeft)
			{
				PointF pt5 = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y);
				PointF pt6 = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
				Graphics.DrawLine(pen, pt5, pt6);
			}
			if (isOverlapRight)
			{
				PointF pt7 = new PointF(end.X - borderLineWidthArray[0] / 2f, end.Y);
				PointF pt8 = new PointF(end.X - borderLineWidthArray[0] / 2f, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2]);
	...
	private void DrawTripleLineBetweenBorder(Border betweenBorder, PointF start, PointF end, bool isLeftBorderSame, bool isRightBorderSame, Border leftBorder, Border rightBorder, bool isOverlapLeft, bool isOverlapRight)
	{
		...
		if (isLeftBorderSame)
		{
			pt3 = new PointF(pt3.X + borderLineWidthArray[0] + borderLineWidthArray[1] + (isOverlapLeft ? 0f : (borderLineWidthArray[2] + borderLineWidthArray[3])), pt3.Y);
		}
		else if (array != null)
		{
			pt3 = new PointF(pt3.X + GetLeftRightLineWidht(array, isLeft: true), pt3.Y);
		...
		if (isRightBorderSame)
		{
			pt4 = new PointF(pt4.X - ((isOverlapRight ? 0f : (borderLineWidthArray[1] + borderLineWidthArray[2])) + borderLineWidthArray[3] + borderLineWidthArray[4]), pt4.Y);
		}
		else if (array2 != null)
		{
			pt4 = new PointF(pt4.X - GetLeftRightLineWidht(array2, isLeft: false), pt4.Y);
		...
		Graphics.DrawLine(pen2, pt3, pt4);
		IPen pen3 = GetPen(betweenBorder.BorderType, borderLineWidthArray[4], betweenBorder.Color);
		PointF pt5 = new PointF(start.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f);
		PointF pt6 = new PointF(end.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4] / 2f);
		if (isLeftBorderSame)
		{
		...
		{
			pt5 = new PointF(pt5.X + GetLeftRightLineWidht(array, isLeft: true), pt5.Y);
		}
		if (isRightBorderSame)
		{
			pt6 = new PointF(pt6.X - (borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]), pt6.Y);
		...
		{
			pt6 = new PointF(pt6.X - GetLeftRightLineWidht(array2, isLeft: false), pt6.Y);
		}
		Graphics.DrawLine(pen3, pt5, pt6);
		if (isOverlapRight || isOverlapLeft)
		{
			if (isOverlapLeft)
			{
				PointF pt7 = new PointF(start.X + borderLineWidthArray[0] / 2f, start.Y);
				PointF pt8 = new PointF(pt7.X, start.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]);
				Graphics.DrawLine(pen, pt7, pt8);
				pt7 = new PointF(start.X + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f, start.Y);
				pt8 = new PointF(pt7.X, pt8.Y);
				Graphics.DrawLine(pen, pt7, pt8);
			}
			if (isOverlapRight)
			{
				PointF pt9 = new PointF(end.X - borderLineWidthArray[0] / 2f, end.Y);
				PointF pt10 = new PointF(pt9.X, end.Y + borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] + borderLineWidthArray[3] + borderLineWidthArray[4]);
				Graphics.DrawLine(pen, pt9, pt10);
				pt9 = new PointF(end.X - (borderLineWidthArray[0] + borderLineWidthArray[1] + borderLineWidthArray[2] / 2f), end.Y);
				pt10 = new PointF(pt9.X, pt10.Y);
				Graphics.DrawLine(pen, pt9, pt10);
			}
		}
	}
	...
	{
		using ISolidBrush brush = CreateSolidBrush(bgColor);
		Graphics.FillRectangle(brush, new Rectangle(0, 0, width, height));
	}

	internal void DrawBackgroundImage(IImage image, WPageSetup pageSetup)
	{
		Graphics.DrawImage(GetImage(image), 0f, 0f, pageSetup.PageSize.Width, pageSetup.PageSize.Height);
	}
	...
	internal void Draw(Page page)
	{
		...
		if (page.PageWidgets.Count != 0)
		{
			...
			{
				DrawBackgroundImage(CreateBitmap().Decode(page.BackgroundImage), page.Setup);
			}
			...
			{
				Graphics.FillRectangle(CreateSolidBrush(Color.FromArgb(240, 240, 240)), new RectangleF(num, 0f, 250f, page.DocSection.PageSetup.PageSize.Height));
			}
		}
		bool flag2 = true;
		bool flag3 = false;
		for (int j = 0; j < page.PageWidgets.Count; j++)
		{
			...
			for (int m = 0; m < page.TrackChangesMarkups.Count; m++)
			{
				...
				RectangleF rect = new RectangleF(num + 20f, trackChangesMarkups.LtWidget.Bounds.Y, 210f, trackChangesMarkups.LtWidget.Bounds.Height);
				float num2 = ((trackChangesMarkups is CommentsMarkups) ? (trackChangesMarkups as CommentsMarkups).ExtraSpacing : 0f);
				PointF pt = new PointF(trackChangesMarkups.Position.X, trackChangesMarkups.Position.Y - num2);
				PointF pointF = new PointF(num - 5f, trackChangesMarkups.Position.Y - num2);
				PointF pt2 = new PointF(num + 20f, trackChangesMarkups.LtWidget.Bounds.Y + 10f);
				...
				IPen pen = CreatePen(color, 1f);
				IPen pen2 = CreatePen(color, 0.5f);
				pen2.DashStyle = DashStyle.Dot;
				Graphics.DrawLine(pen2, pt, pointF);
				Graphics.DrawLine(pen2, pointF, pt2);
				DrawRoundedRectangle(pen, rect, (trackChangesMarkups is CommentsMarkups) ? GetRevisionFillColor(revisionOptions.CommentColor, flag4) : Color.White);
				if (!(trackChangesMarkups is CommentsMarkups))
				{
	...
	private void DrawRoundedRectangle(IPen pen, RectangleF rect, Color fillColor)
	{
		...
		Graphics.DrawPath(pen, graphicsPath);
		Graphics.FillPath(CreateSolidBrush(fillColor), graphicsPath);
	}

	private void DrawMarkupTriangles(PointF position, Color revisionColor)
	{
		IBrush brush = CreateSolidBrush(revisionColor);
		IGraphicsPath graphicsPath = CreateGraphicsPath();
		...
		graphicsPath.AddLine(position, pointF);
		graphicsPath.CloseFigure();
			...
			else if (layoutedWidget2 != null)
			{
				if (ownerLayoutedWidget is WTextBox || ownerLayoutedWidget is Shape || ownerLayoutedWidget is ChildShape)
				{
					...
					if (ownerLayoutedWidget is WTextBox)
					{
						...
						flag = wTextBox.TextBoxFormat.FlipHorizontal;
					...
					else if (ownerLayoutedWidget is Shape)
					{
						...
						num2 = obj.Rotation;
						flag = obj.FlipHorizontal;
						flag2 = obj.FlipVertical;
					}
					else if (ownerLayoutedWidget is ChildShape)
					{
						ChildShape obj2 = ownerLayoutedWidget as ChildShape;
						num2 = obj2.RotationToRender;
						flag = obj2.FlipHorizantalToRender;
						flag2 = obj2.FlipVerticalToRender;
					}
					if (num2 != 0f || (num2 == 0f && (flag || flag2)))
					{
				...
				if ((layoutedWidget3.Widget is WTextRange || layoutedWidget3.Widget is SplitStringWidget) && !IsRTLParaLine(layoutedWidget2) && !HasTextRangeBidi(layoutedWidget2.ChildWidgets) && layoutedWidget3.Bounds.Width > 0f)
				{
					...
					if (wTextRange != null && i != num - 1)
					{
						...
						float num3 = 0f;
						if (underLineValues != null && underLineValues.Count != 0 && wTextRange.CharacterFormat.UnderlineStyle != 0 && wTextRange.CharacterFormat.UnderlineStyle != UnderlineStyle.Words)
						{
	...
	private void DrawLine(Dictionary<WCharacterFormat, List<RectangleF>> underLineValues, Dictionary<WCharacterFormat, RectangleF> strikeThroughValues, LayoutedWidget ltWidget)
	{
		if (underLineValues != null)
		{
			foreach (KeyValuePair<WCharacterFormat, List<RectangleF>> underLineValue in underLineValues)
			{
				...
				if (isNeedToScale || isRotateTransformApplied)
				{
					ResetTransform();
				}
				if (isNeedToClip)
		...
		foreach (KeyValuePair<WCharacterFormat, RectangleF> strikeThroughValue in strikeThroughValues)
		{
			...
			if (isNeedToScale2 || isRotateTransformApplied2)
			{
				ResetTransform();
			}
			if (isNeedToClip2)
	...
	private void DrawUnderLine(WCharacterFormat characterFormat, RectangleF textBounds)
	{
		...
		else if (characterFormat.UnderlineStyle == UnderlineStyle.Double)
		{
			y = textBounds.Y + GetAscent(font2, FontScriptType.English) + 1.25f * num2;
		}
		IPen pen = CreatePen(characterFormat, num2);
		if (characterFormat.UnderlineStyle == UnderlineStyle.Wavy || characterFormat.UnderlineStyle == UnderlineStyle.WavyHeavy)
		{
			Graphics.DrawPath(pen, CreateWavyPath(characterFormat, textBounds, font));
		}
		else if (characterFormat.UnderlineStyle == UnderlineStyle.WavyDouble)
		{
			Graphics.DrawPath(pen, CreateWavyPath(characterFormat, textBounds, font));
			Graphics.DrawPath(pen, CreateWavyPath(characterFormat, new RectangleF(textBounds.X, textBounds.Y + num2 / 1.8f, textBounds.Width, textBounds.Height), font));
		}
		else
		{
			Graphics.DrawLine(pen, new PointF(x, y), new PointF(x + textBounds.Width, y));
		}
	}

	private IGraphicsPath CreateWavyPath(WCharacterFormat characterFormat, RectangleF bounds, DocGen.Drawing.Font font)
	{
		IGraphicsPath graphicsPath = CreateGraphicsPath();
		double num = ((characterFormat.UnderlineStyle == UnderlineStyle.WavyDouble) ? (bounds.Width / 1.2f) : bounds.Width);
		PointF[] array = new PointF[(int)num + 1];
		float num2 = bounds.X;
		DocGen.Drawing.Font font2 = ((characterFormat.SubSuperScript != 0) ? characterFormat.Document.FontSettings.GetFont(font.Name, font.Size / 1.6f, font.Style, FontScriptType.English) : font);
		float num3 = bounds.Y + font2.Size;
		for (int i = 0; (double)i <= num; i++)
		{
			PointF pointF = new PointF(num2, num3);
			array[i] = pointF;
			num2 += ((characterFormat.UnderlineStyle == UnderlineStyle.WavyDouble) ? 1.2f : 1f);
			num3 = ((num3 == bounds.Y + font2.Size) ? ((characterFormat.UnderlineStyle == UnderlineStyle.WavyDouble) ? (bounds.Y + font2.Size + (float)Math.Sqrt(font2.Size) / 9f) : (bounds.Y + font2.Size + (float)Math.Sqrt(font2.Size) / 2.5f)) : (bounds.Y + font2.Size));
		}
	...
	private void DrawStrikeThrough(WCharacterFormat characterFormat, RectangleF textBounds)
	{
		...
		if (characterFormat.Strikeout)
		{
			...
			IPen pen = CreatePen(characterFormat, num);
			Graphics.DrawLine(pen, new PointF(x, y), new PointF(x + textBounds.Width, y));
		}
	...
	private void SetClip(RectangleF clippingBounds)
	{
		...
		{
			Graphics.SetClip(GetClipBounds(clippingBounds, clippingBounds.Width, 0f), CombineMode.Replace);
		}
	}
	...
	{
		Graphics.Transform = GetTransformMatrix(rotateBounds, rotation, flipH, flipV);
	}

	public void SetScaleTransform(float sx, float sy)
	{
		Matrix matrix = new Matrix();
		WordDocument.RenderHelper.ApplyScale(matrix, sx, sy);
		Graphics.Transform = matrix;
	}
	...
	private void DrawCommentHighlighter(RevisionOptions revisionOptions, RectangleF bounds, bool isResolvedComment)
	{
		if (bounds.Width > 0f)
		{
			...
			ISolidBrush brush = CreateSolidBrush(revisionFillColor);
	...
	private void DrawLineBackGroundColors(WParagraph paragraph, LayoutedWidget ltWidget)
	{
		...
		else if (!paragraphFormat.BackColor.IsEmpty)
		{
			...
			{
				Graphics.FillRectangle(CreateSolidBrush(paragraphFormat.BackColor), backGroundColorRenderingBounds[j]);
			}
		}
	...
	internal void DrawChart(WChart chart, LayoutedWidget widget)
	{
		...
		WordDocument.RenderHelper.ConvertChartAsImage(chart.OfficeChart, memoryStream, ChartRenderingOptions);
		...
		{
			Graphics.DrawImage(GetImage(CreateImageFromStream(memoryStream)), bounds);
			Graphics.ResetTransform();
		}
	...
	{
		if (base.PreserveFormField)
		{
			m_editableFormFieldinEMF.Add(ltWidget);
		}
		else
		{
	...
	internal void Draw(IStringWidget stringWidget, LayoutedWidget ltWidget, string text)
	{
		...
		if (m_editableTextFormTextRange != null)
		{
			...
			string text2 = "";
			if (m_editableTextFormTextRange.PreviousSibling is WFieldMark)
			{
				WFieldMark wFieldMark2 = m_editableTextFormTextRange.PreviousSibling as WFieldMark;
				if (wFieldMark2.ParentField is WTextFormField)
				{
					text2 = (wFieldMark2.ParentField as WTextFormField).Text;
				}
			}
			...
			{
				int index = m_editableFormFieldinEMF.Count - 1;
				m_editableFormFieldinEMF[index].Bounds = new RectangleF(m_editableFormFieldinEMF[index].Bounds.X, m_editableFormFieldinEMF[index].Bounds.Y, m_editableTextFormBounds.Width, m_editableFormFieldinEMF[index].Bounds.Height);
	...
	private void DrawTextWatermark(TextWatermark textWatermark, RectangleF bounds, WPageSetup pageSetup)
	{
		...
		IBitmap bitmap = ConvertAsImage(textWatermark, font);
		IColorMatrix colorMatrix = CreateColorMatrix();
		if (textWatermark.Semitransparent)
		{
			colorMatrix.Matrix33 = 0.5f;
		}
		IImageAttributes imageAttributes = CreateImageAttributes();
		imageAttributes.SetColorMatrix(colorMatrix, ColorMatrixFlag.Default, ColorAdjustType.Bitmap);
		if (textWatermark.Rotation != 0)
		{
			bounds = new RectangleF(bounds.X, bounds.Y, bounds.Width, bounds.Height);
			Graphics.Transform = GetTransformMatrix(bounds, textWatermark.Rotation);
		...
		memoryStream.Position = 0L;
		bitmap = CreateBitmap().Decode(memoryStream.ToArray()) as IBitmap;
		Graphics.DrawImage(GetImage(bitmap), destRect, 0f, 0f, bitmap.Width, bitmap.Height, GraphicsUnit.Pixel, imageAttributes);
		if (textWatermark.Rotation != 0)
		{
			ResetTransform();
		}
	...
	private IBitmap ConvertAsImage(TextWatermark textWatermark, DocGen.Drawing.Font font)
	{
		IBitmap bmp = CreateBitmap(1, 1);
		IGraphics graphicsFromImage = GetGraphicsFromImage(bmp);
		...
		IGraphics graphicsFromImage2 = GetGraphicsFromImage(bmp);
		...
		graphicsFromImage2.DrawString(solidBrush: CreateBrush(textWatermark.Color), text: textWatermark.Text, font: font, x: 0, y: 0, fontStyle: font.Style, scriptType: FontScriptType.English);
	...
	private void AdjustPictureBrightnessAndContrast(IBitmap bmp, WPicture picture, ref IImageAttributes imageAttributes, bool isWatermark)
	{
		float num = 0f;
		float num2 = 0f;
		float num3 = 1f;
		...
		{
			num = picture.FillFormat.BlipFormat.ImageEffect.Brightness / 100f;
			num2 = 2f * picture.FillFormat.BlipFormat.ImageEffect.Contrast / 100f;
		}
		float[][] newColorMatrix = new float[5][]
		{
			new float[5] { num2, 0f, 0f, 0f, 0f },
			new float[5] { 0f, num2, 0f, 0f, 0f },
			new float[5] { 0f, 0f, num2, 0f, 0f },
			new float[5] { 0f, 0f, 0f, 1f, 0f },
			new float[5] { num, num, num, 0f, 1f }
		};
		imageAttributes = CreateImageAttributes();
		imageAttributes.SetColorMatrix(CreateColorMatrix(newColorMatrix), ColorMatrixFlag.Default, ColorAdjustType.Bitmap);
		imageAttributes.SetGamma(num3, ColorAdjustType.Bitmap);
	}

	private void DrawImageWatermark(PictureWatermark pictureWatermark, RectangleF bounds, WPageSetup pageSetup)
	{
		IImageAttributes imageAttributes = null;
		bool num = pictureWatermark.Picture.RawFormat.Equals(DocGen.DocIO.DLS.Entities.ImageFormat.Tiff);
		IBitmap bitmap = null;
		...
		bitmap = CreateBitmap(1, 1).Decode(array) as IBitmap;
		...
		if (pictureWatermark.Washout)
		{
			...
			Graphics.DrawImage(GetImage(bitmap), new Rectangle((int)bounds.X, (int)bounds.Y, (int)bounds.Width, (int)bounds.Height), 0f, 0f, bitmap.Width, bitmap.Height, GraphicsUnit.Pixel, imageAttributes);
		}
		else
		{
			Graphics.DrawImage(GetImage(array), new Rectangle((int)bounds.X, (int)bounds.Y, (int)bounds.Width, (int)bounds.Height));
		}
	}

	internal void DrawCheckBox(WCheckBox checkbox, LayoutedWidget ltWidget)
	{
		IPen pen = CreatePen(GetTextColor(checkbox.CharacterFormat));
		pen.Width = 0.7f;
		...
		if (checkbox.Checked)
		{
			PointF pt = new PointF(rectangleF.X, rectangleF.Y);
			PointF pt2 = new PointF(rectangleF.Right, rectangleF.Bottom);
			Graphics.DrawLine(pen, pt, pt2);
			pt = new PointF(rectangleF.Right, rectangleF.Top);
	...
	internal void DrawShape(Shape shape, LayoutedWidget ltWidget)
	{
		RectangleF rectangleF = ltWidget.Bounds;
		ResetTransform();
		Color color = ((!shape.LineFormat.Color.IsEmpty) ? shape.LineFormat.Color : ((!shape.LineFormat.ForeColor.IsEmpty) ? shape.LineFormat.ForeColor : Color.Black));
		IPen pen = CreatePen(color);
		if (shape.AutoShapeType != DocGen.DocIO.DLS.AutoShapeType.StraightConnector && shape.AutoShapeType != DocGen.DocIO.DLS.AutoShapeType.ElbowConnector)
		{
		...
		IGraphicsPath graphicsPath = GetGraphicsPath(shape, rectangleF, ref pen);
		...
		if (graphicsPath.PointCount > 0)
		{
			...
			{
				Color color2 = ((!shape.FillFormat.Color.IsEmpty && shape.FillFormat.Transparency != 100f) ? shape.FillFormat.Color : ((!shape.FillFormat.ForeColor.IsEmpty) ? shape.FillFormat.ForeColor : Color.Empty));
				...
				{
					Graphics.FillPath(CreateSolidBrush(color2), graphicsPath);
				}
			}
			if (shape.LineFormat.Line)
			{
				pen.LineJoin = GetLineJoin(shape.LineFormat.LineJoin);
	...
	private void DrawPictureFill(ImageRecord imageRecord, IGraphicsPath path, RectangleF bounds, float transparency)
	{
		...
		if (imageRecord.ImageFormat.Equals(DocGen.DocIO.DLS.Entities.ImageFormat.Tiff))
		{
			array = WordDocument.RenderHelper.ConvertTiffToPng(array);
		}
		MemoryStream memoryStream = new MemoryStream(array);
		IImage image = CreateImageFromStream(memoryStream);
		IImageAttributes imageAttributes = CreateImageAttributes();
		ApplyImageTransparency(imageAttributes, transparency);
		if (path != null)
		{
			IBrush textureBrush = WordDocument.RenderHelper.GetTextureBrush(image, new Rectangle((int)bounds.X, (int)bounds.Y, (int)bounds.Width, (int)bounds.Height), imageAttributes);
			Graphics.FillPath(textureBrush, path);
			textureBrush.Dispose();
		}
		else
		{
			Graphics.DrawImage(image, Rectangle.Round(bounds), 0f, 0f, image.Width, image.Height, GraphicsUnit.Pixel, imageAttributes);
		}
		image.Dispose();
		memoryStream.Dispose();
	}
	...
	private void DrawLineShapeBasedOnLineType(Shape shape, RectangleF bounds, IPen pen)
	{
		...
		graphicsPath.AddLines(linePointsBasedOnFlip);
		Graphics.DrawPath(pen, graphicsPath);
		...
		IGraphicsPath graphicsPath2 = CreateGraphicsPath();
		graphicsPath2.AddLines(linePointsBasedOnFlip);
	...
	private void Rotate(ParagraphItem shapeFrame, float rotation, bool flipV, bool flipH, RectangleF rect)
	{
		...
		{
			Graphics.Transform = GetTransformMatrix(rect, num, flipV, flipH);
		}
	}
	...
	private IGraphicsPath GetGraphicsPath(Shape shape, RectangleF bounds, ref IPen pen)
	{
		ShapePath shapePath = new ShapePath(bounds, shape.ShapeGuide);
		IGraphicsPath path = CreateGraphicsPath();
		...
		{
			IGraphicsPath[] horizontalScroll = shapePath.GetSmileyFacePath();
			int num = 0;
			...
			IGraphicsPath[] array = horizontalScroll;
			foreach (IGraphicsPath graphicsPath2 in array)
			{
				if (num == 2)
				{
				...
				if (shape.FillFormat.FillType == FillType.FillPicture && shape.FillFormat.ImageRecord != null)
				{
					DrawPictureFill(shape.FillFormat.ImageRecord, graphicsPath2, bounds, shape.FillFormat.BlipFormat.Transparency);
				}
				else if (color != Color.Empty)
				{
					Graphics.FillPath(CreateSolidBrush(color), graphicsPath2);
				}
				Graphics.DrawPath(pen, graphicsPath2);
			}
			break;
		...
		{
			IGraphicsPath[] horizontalScroll = shapePath.GetArcPath();
			Color color = ((!shape.FillFormat.Color.IsEmpty) ? shape.FillFormat.Color : ((!shape.FillFormat.ForeColor.IsEmpty) ? shape.FillFormat.ForeColor : Color.Empty));
			if (shape.FillFormat.FillType == FillType.FillGradient && shape.FillFormat.GradientFill != null && shape.FillFormat.GradientFill.GradientStops.Count > 0)
			{
				color = shape.FillFormat.GradientFill.GradientStops[shape.FillFormat.GradientFill.GradientStops.Count - 1].Color;
			...
			{
				Graphics.FillPath(CreateSolidBrush(color), horizontalScroll[1]);
			}
			if (shape.LineFormat.Line)
			{
		...
		{
			IGraphicsPath[] horizontalScroll = shapePath.GetHorizontalScroll();
			Color color = ((!shape.FillFormat.Color.IsEmpty) ? shape.FillFormat.Color : ((!shape.FillFormat.ForeColor.IsEmpty) ? shape.FillFormat.ForeColor : Color.Empty));
			if (shape.FillFormat.FillType == FillType.FillGradient && shape.FillFormat.GradientFill != null && shape.FillFormat.GradientFill.GradientStops.Count > 0)
			{
				color = shape.FillFormat.GradientFill.GradientStops[shape.FillFormat.GradientFill.GradientStops.Count - 1].Color;
			...
			foreach (IGraphicsPath graphicsPath in array)
			{
				if (shape.FillFormat.FillType == FillType.FillPicture && shape.FillFormat.ImageRecord != null)
				{
					DrawPictureFill(shape.FillFormat.ImageRecord, graphicsPath, bounds, shape.FillFormat.BlipFormat.Transparency);
				}
				else if (color != Color.Empty)
				{
					Graphics.FillPath(CreateSolidBrush(color), graphicsPath);
	...
	private IGraphicsPath GetGraphicsPath(ChildShape shape, RectangleF bounds, ref IPen pen)
	{
		ShapePath shapePath = new ShapePath(bounds, shape.ShapeGuide);
		IGraphicsPath path = CreateGraphicsPath();
		...
		case DocGen.DocIO.DLS.AutoShapeType.ElbowConnector:
			...
			{
				Graphics.Transform = GetTransformMatrix(bounds, 0f, shape.FlipHorizantal, shape.FlipVertical);
			}
			return path;
		case DocGen.DocIO.DLS.AutoShapeType.CurvedConnector:
			return shapePath.GetCurvedConnectorPath();
	...
	public void IntializeGraphics(WPageSetup pageSetup)
	{
		...
		IBitmap bitmap = WordDocument.RenderHelper.GetBitmap(width, height);
		Graphics = WordDocument.RenderHelper.GetGraphics(bitmap);
		Graphics.PageUnit = GraphicsUnit.Point;
	}
	...
	{
		return WordDocument.RenderHelper.GetGraphics(bmp);
	}

	private void DrawUnicodeString(string text, DocGen.Drawing.Font font, IBrush brush, RectangleF rectangle, StringFormat stringFormat, FontScriptType scriptType)
	{
		Graphics.DrawString(text, font, brush, rectangle, stringFormat, scriptType);
	}

	private IImage GetImage(IImage image)
	{
		if (!WordDocument.RenderHelper.HasBitmap(image))
		{
			...
			image = WordDocument.RenderHelper.CreateImageFromStream(memoryStream);
		}
		return image;
	}
	...
	{
		IImage image = WordDocument.RenderHelper.CreateImageFromStream(new MemoryStream(imageBytes));
		if (!WordDocument.RenderHelper.HasBitmap(image))
		{
			Stream manifestResourceStream = GetManifestResourceStream("ImageNotFound.jpg");
			MemoryStream memoryStream = new MemoryStream();
	...
	{
		return WordDocument.RenderHelper.CreateImageFromStream(stream);
	}

	private IHatchBrush CreateHatchBrush(HatchStyle hatchstyle, Color foreColor, Color backColor)
	{
		return WordDocument.RenderHelper.GetHatchBrush(hatchstyle, foreColor, backColor);
	}

	private IPen CreatePen(Color color)
	{
		return WordDocument.RenderHelper.GetPen(color);
	}

	private IBitmap CreateBitmap()
	{
		return WordDocument.RenderHelper.GetBitmap();
	}

	private IBrush CreateBrush(Color color)
	{
		return WordDocument.RenderHelper.GetSolidBrush(color);
	}

	private IPen CreatePen(Color color, float width)
	{
		return WordDocument.RenderHelper.GetPen(color, width);
	}
	...
	{
		return WordDocument.RenderHelper.GetColorMatrix();
	}

	private IColorMatrix CreateColorMatrix(float[][] newColorMatrix)
	{
		return WordDocument.RenderHelper.GetColorMatrix(newColorMatrix);
	}

	private IImageAttributes CreateImageAttributes()
	{
		return WordDocument.RenderHelper.GetImageAttributes();
	}

	private void DrawArrowHead(ChildShape shape, IPen pen, RectangleF bounds, ref bool isArrowHeadExist, ref IGraphicsPath path, PointF[] linePoints)
	{
	...
	private void DrawArrowHead(Shape shape, IPen pen, RectangleF bounds, ref bool isArrowHeadExist, ref IGraphicsPath path, PointF[] linePoints)
	{
		isArrowHeadExist = false;
		PointF endPoint = new PointF(0f, 0f);
		switch (shape.LineFormat.EndArrowheadStyle)
	...
	private void DrawOpenEndArrowHead(Shape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
	...
	private void DrawOpenEndArrowHead(ChildShape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
		PointF[] array = FindArrowHeadPoints(shape, pen, bounds, linePoints, isFromOpenArrow: true, isFromBeginArrow: false);
		if (shape.LineFormat.BeginArrowheadStyle != ArrowheadStyle.ArrowheadTriangle && shape.LineFormat.BeginArrowheadStyle != ArrowheadStyle.ArrowheadOpen)
		{
	...
	private void DrawCloseEndArrowHead(Shape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
	...
	private void DrawStealthEndArrowHead(Shape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
		PointF[] array = FindArrowHeadPoints(shape, pen, bounds, linePoints, isFromOpenArrow: false, isFromBeginArrow: false);
		if (shape.LineFormat.BeginArrowheadStyle != ArrowheadStyle.ArrowheadTriangle && shape.LineFormat.BeginArrowheadStyle != ArrowheadStyle.ArrowheadOpen)
		{
	...
	private void DrawCloseEndArrowHead(ChildShape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
		PointF[] array = FindArrowHeadPoints(shape, pen, bounds, linePoints, isFromOpenArrow: false, isFromBeginArrow: false);
		if (shape.LineFormat.BeginArrowheadStyle != ArrowheadStyle.ArrowheadTriangle && shape.LineFormat.BeginArrowheadStyle != ArrowheadStyle.ArrowheadOpen)
		{
	...
	private void DrawStealthEndArrowHead(ChildShape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
		PointF[] array = FindArrowHeadPoints(shape, pen, bounds, linePoints, isFromOpenArrow: false, isFromBeginArrow: false);
		if (shape.LineFormat.BeginArrowheadStyle != ArrowheadStyle.ArrowheadTriangle && shape.LineFormat.BeginArrowheadStyle != ArrowheadStyle.ArrowheadOpen)
		{
	...
	private void DrawOpenBeginArrowHead(Shape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
	...
	private void DrawOpenBeginArrowHead(ChildShape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
		PointF[] array = FindArrowHeadPoints(shape, pen, bounds, linePoints, isFromOpenArrow: true, isFromBeginArrow: true);
		path.StartFigure();
		if (endPoint.X == 0f && endPoint.Y == 0f)
	...
	private void DrawCloseBeginArrowHead(Shape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
	...
	private void DrawCloseBeginArrowHead(ChildShape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
		PointF[] array = FindArrowHeadPoints(shape, pen, bounds, linePoints, isFromOpenArrow: false, isFromBeginArrow: true);
		path.StartFigure();
		if (endPoint.X == 0f && endPoint.Y == 0f)
	...
	private void DrawStealthBeginArrowHead(Shape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
		PointF[] array = FindArrowHeadPoints(shape, pen, bounds, linePoints, isFromOpenArrow: false, isFromBeginArrow: true);
		path.StartFigure();
		if (endPoint.X == 0f && endPoint.Y == 0f)
	...
	private void DrawStealthBeginArrowHead(ChildShape shape, IPen pen, RectangleF bounds, PointF[] linePoints, ref PointF endPoint, ref bool isArrowHeadExist, ref IGraphicsPath path)
	{
		PointF[] array = FindArrowHeadPoints(shape, pen, bounds, linePoints, isFromOpenArrow: false, isFromBeginArrow: true);
		path.StartFigure();
		if (endPoint.X == 0f && endPoint.Y == 0f)
	...
	private void AddOpenArrowHeadPoints(PointF[] points, ref IGraphicsPath path)
	{
	...
	private void GetArrowDefaultValues(LineFormat lineFormat, IPen pen, ref float arrowLength, ref float arrowAngle, ref float adjustValue, bool isFromOpenArrow, bool isFromBeginArrow)
	{
		if (isFromOpenArrow)
		{
	...
	private IPen GetPen(Border border, bool isParagraphBorder)
	{
		...
		pen = CreatePen(color, border.LineWidth);
		switch (border.BorderType)
		{
		...
		if (!isParagraphBorder)
		{
			pen.StartCap = DocGen.Drawing.LineCap.Square;
			pen.EndCap = DocGen.Drawing.LineCap.Square;
		}
		return pen;
	}
	...
	private IPen GetPen(BorderStyle borderType, float borderLineWidth, Color borderColor)
	{
		...
		pen = CreatePen(color, borderLineWidth);
		switch (borderType)
		{
		case BorderStyle.DashLargeGap:
			pen.DashPattern = new float[2] { 3f, 5f };
	...
	private IPen GetPen(UnderlineStyle underlineStyle, float lineWidth, Color lineColor)
	{
		...
		case UnderlineStyle.DottedHeavy:
			pen.Width = lineWidth * 1.75f;
			pen.DashPattern = new float[2] { 1.2f, 1f };
			break;
		...
		case UnderlineStyle.DotDashHeavy:
			pen.Width = lineWidth * 1.75f;
			pen.DashPattern = new float[4] { 2.25f, 1.03f, 0.5f, 1.03f };
			break;
		case UnderlineStyle.DotDotDash:
			pen.DashPattern = new float[6] { 8f, 3f, 1f, 3f, 1f, 3f };
			break;
		case UnderlineStyle.DotDotDashHeavy:
			pen.Width = lineWidth * 1.45f;
			pen.DashPattern = new float[6] { 3.25f, 1.45f, 1f, 1.45f, 1f, 1.45f };
			break;
		case UnderlineStyle.Thick:
			pen.Width = lineWidth * 1.819375f;
			break;
		case UnderlineStyle.Double:
			pen.Width = lineWidth * 2.55f;
			pen.CompoundArray = new float[4] { 0f, 0.21f, 0.79f, 1f };
			break;
	...
	private IImage ScaleImage(IImage srcImage, float width, float height)
	{
		...
		int width2 = srcImage.Width;
		int height2 = srcImage.Height;
		int num = 0;
		int num2 = 0;
		int num3 = (int)width;
		int num4 = (int)height;
		...
		IBitmap bitmap = CreateBitmap(num3, num4);
		bitmap.SetResolution(srcImage.HorizontalResolution, srcImage.VerticalResolution);
		int num5 = 0;
		int num6 = 0;
		using (IGraphics graphics = GetGraphicsFromImage(bitmap))
		{
			graphics.InterpolationMode = InterpolationMode.HighQualityBicubic;
			graphics.DrawImage(GetImage(srcImage), new RectangleF(num, num2, num3, num4), new RectangleF(num5, num6, width2, height2), GraphicsUnit.Pixel);
		}
		return null;
	}
