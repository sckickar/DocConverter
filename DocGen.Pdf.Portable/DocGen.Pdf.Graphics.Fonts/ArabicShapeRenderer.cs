...
internal class ArabicShapeRenderer
{
	...
	internal Dictionary<char, char[]> ArabicMapTable => m_arabicMapTable;
	...
	private char GetCharacterShape(char input, int index)
	{
		if (input >= 'ء' && input <= 'ۓ')
		{
			if (m_arabicMapTable.TryGetValue(input, out char[] value))
			{
	...
	private int GetShapeCount(char shape)
	{
		...
		{
			if (m_arabicMapTable.TryGetValue(shape, out char[] value))
			{
