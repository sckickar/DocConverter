...
internal class LookupTable4 : LookupTable
{
	...
	internal override bool ReplaceGlyph(OtfGlyphInfoList glyphList)
	{
		...
		else
		{
			...
			{
				item.Records.TryGetValue(glyphList.Glyphs[glyphList.Index].Index, out GPOSRecord value);
				if (value == null)
				{
					continue;
				}
				...
				item.Collection.TryGetValue(glyphInfoIndex.GlyphInfo.Index, out GPOSValueRecord[] value2);
				if (value2 != null)
				{
					int index = value.Index;
					GPOSValueRecord gPOSValueRecord = value2[index];
					GPOSValueRecord record = value.Record;
					if (gPOSValueRecord != null && record != null)
					{
