...
public class AstVisitor
{
	...
	{
		foreach (IStatementListItem item in program.Body)
		{
			VisitStatement((Statement)item);
		}
	}
	...
	{
		foreach (INode param in functionDeclaration.Params)
		{
			Visit(param);
		}
	...
	{
		foreach (VariableDeclarator declaration in variableDeclaration.Declarations)
		{
			Visit(declaration);
		}
	}
	...
	protected virtual void VisitSwitchCase(SwitchCase switchCase)
	{
		...
		foreach (IStatementListItem item in switchCase.Consequent)
		{
			VisitStatement(item.As<Statement>());
		}
	}
	...
	{
		Identifier expression = ((forInStatement.Left.Type == Nodes.VariableDeclaration) ? forInStatement.Left.As<VariableDeclaration>().Declarations.First().Id.As<Identifier>() : forInStatement.Left.As<Identifier>());
	...
	protected virtual void VisitArrowFunctionExpression(ArrowFunctionExpression arrowFunctionExpression)
	{
		...
		{
			blockStatement = arrowFunctionExpression.Body.As<BlockStatement>();
		}
		else
		{
			NodeList<IStatementListItem> body = new NodeList<IStatementListItem>(new IStatementListItem[1]
			{
				new ReturnStatement(arrowFunctionExpression.Body.As<Expression>())
			}, 1);
			blockStatement = new BlockStatement(in body);
		}
		BlockStatement body2 = blockStatement;
		FunctionExpression function = new FunctionExpression(new Identifier(null), in arrowFunctionExpression.Params, body2, generator: false, IsStrictMode, async: false, new HoistingScope());
	...
	{
		foreach (ObjectExpressionProperty property in objectExpression.Properties)
		{
	...
	{
		foreach (ArgumentListElement argument in newExpression.Arguments)
		{
			VisitExpression(argument.As<Expression>());
		}
	...
	{
		foreach (INode param in function.Params)
		{
			Visit(param);
		}
		Visit(function.Body);
	...
	protected virtual void VisitCallExpression(CallExpression callExpression)
	{
		...
		foreach (ArgumentListElement argument in callExpression.Arguments)
		{
			VisitExpression(argument.As<Expression>());
		}
	}
	...
	protected virtual void VisitArrayExpression(ArrayExpression arrayExpression)
	{
		...
		{
			VisitExpression(element.As<Expression>());
		}
	}
	...
	{
		foreach (IStatementListItem item in blockStatement.Body)
		{
			VisitStatement(item.As<Statement>());
		}
	}
