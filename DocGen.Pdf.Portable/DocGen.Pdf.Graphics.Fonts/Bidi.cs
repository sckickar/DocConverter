...
internal class Bidi
{
	...
	private Dictionary<int, List<int>> m_ranges;

	internal bool m_isVisualOrder = true;
	...
	internal byte[] IndexLevels
	{
		get
		{
			return m_indexLevels;
		}
		set
		{
			m_indexLevels = value;
	...
	private OtfGlyphInfo[] DoMirrorShaping(OtfGlyphInfo[] text, TtfReader ttfReader)
	{
		...
		for (int i = 0; i < text.Length; i++)
		{
			char key = (char)((text[i].CharCode > 0) ? text[i].CharCode : ((text[i].Characters != null) ? text[i].Characters[0] : '\0'));
	...
	internal string GetLogicalToVisualString(string inputText, bool isRTL)
	{
		...
		m_ranges = new Dictionary<int, List<int>>();
		...
		if (m_isVisualOrder)
		{
			DoBidiOrder(0, m_indexLevels.Length - 1);
			if (m_ranges.Count > 0)
			{
				foreach (KeyValuePair<int, List<int>> range in m_ranges)
				{
					if (m_ranges.Last().Key != range.Key)
					{
						while (range.Value[0] - 1 != -1 && (rTLCharacters.Types[range.Value[0] - 1] == 17 || rTLCharacters.Types[range.Value[0] - 1] == 18 || rTLCharacters.Types[range.Value[0] - 1] == 12))
						{
							range.Value[0] = range.Value[0] - 1;
						}
						while (range.Value[1] != m_ranges.Last().Value[1] + 1 && range.Value[1] < rTLCharacters.Types.Length && (rTLCharacters.Types[range.Value.Last()] == 17 || rTLCharacters.Types[range.Value.Last()] == 18 || rTLCharacters.Types[range.Value.Last()] == 12))
						{
							range.Value[1] = range.Value.Last() + 1;
						}
					}
				}
				if (m_ranges.Count == 1)
				{
					ReArrange(m_ranges[0][0], m_ranges[0][1]);
				}
				else
				{
					int num = 0;
					...
					{
						List<int> list = m_ranges[i];
						if (list[0] == 0)
						{
							num = list[1];
							continue;
						}
						ReArrange(num, list[0]);
						num = list[1];
					}
					if (num != m_indexLevels.Length)
					{
						ReArrange(num, m_indexLevels.Length);
					}
				}
			}
		}
		...
		for (int j = 0; j < m_indexes.Length; j++)
		{
			int index = m_indexes[j];
	...
	{
		int num = m_indexLevels[sIndex];
		int num2 = num;
		int num3 = num;
		int num4 = num;
		for (int i = sIndex + 1; i <= eIndex; i++)
		{
			int num5 = m_indexLevels[i];
			if (num5 > num)
			{
				num = num5;
			}
		...
		if ((num3 & 1) == 1)
		{
			m_ranges[m_ranges.Count] = new List<int>
			{
				sIndex,
				eIndex + 1
			};
		...
		while (num >= num2)
		{
			...
			while (true)
			{
				if (num6 <= eIndex && m_indexLevels[num6] < num)
				{
					num6++;
					continue;
				}
				...
				int j;
				for (j = num6 + 1; j <= eIndex && m_indexLevels[j] >= num; j++)
				{
				}
				m_ranges[m_ranges.Count] = new List<int> { num6, j };
				num6 = j + 1;
			}
			num--;
		}
	...
	internal OtfGlyphInfo[] GetLogicalToVisualGlyphs(List<OtfGlyphInfo> inputText, bool isRTL, TtfReader ttfReader, byte[] bidi)
	{
		m_indexLevels = new byte[inputText.Count];
		m_indexes = new int[inputText.Count];
		if (bidi != null)
		{
			Array.Copy(bidi, m_indexLevels, m_indexLevels.Length);
		}
		else
		{
			RTLCharacters rTLCharacters = new RTLCharacters();
		...
		int[] array2 = new int[m_indexes.Length];
		for (int j = 0; j < m_indexes.Length; j++)
		{
			array2[m_indexes[j]] = j;
		}
		UpdatePlacementLineGlyphs(m_indexes, array2, list);
		return list.ToArray();
	}

	private void UpdatePlacementLineGlyphs(int[] reorder, int[] inverseReorder, List<OtfGlyphInfo> reorderedLine)
	{
		for (int i = 0; i < reorderedLine.Count; i++)
		{
			OtfGlyphInfo otfGlyphInfo = reorderedLine[i];
			if (otfGlyphInfo.m_placment != 0)
			{
				int num = reorder[i] + otfGlyphInfo.m_placment;
				int num2 = inverseReorder[num] - i;
				if (otfGlyphInfo.m_placment != num2)
				{
					otfGlyphInfo = (reorderedLine[i] = new OtfGlyphInfo(otfGlyphInfo));
					otfGlyphInfo.m_placment = (short)num2;
				}
			}
		}
	}
	...
	{
		if (c >= '־' && c <= 68479 && (c == '־' || c == '׀' || c == '׃' || c == '׆' || ('א' <= c && c <= 'ת') || ('װ' <= c && c <= '״') || c == '؈' || c == '؋' || c == '؍' || c == '؛' || ('؞' <= c && c <= 'ي') || ('٭' <= c && c <= 'ٯ') || ('ٱ' <= c && c <= 'ە') || ('ۥ' <= c && c <= 'ۦ') || ('ۮ' <= c && c <= 'ۯ') || ('ۺ' <= c && c <= '܍') || c == 'ܐ' || ('ܒ' <= c && c <= 'ܯ') || ('ݍ' <= c && c <= 'ޥ') || c == 'ޱ' || ('߀' <= c && c <= 'ߪ') || ('ߴ' <= c && c <= 'ߵ') || c == 'ߺ' || ('ࠀ' <= c && c <= 'ࠕ') || c == 'ࠚ' || c == 'ࠤ' || c == 'ࠨ' || ('࠰' <= c && c <= '࠾') || ('ࡀ' <= c && c <= 'ࡘ') || c == '࡞' || c == '\u200f' || c == 'יִ' || ('ײַ' <= c && c <= 'ﬨ') || ('שׁ' <= c && c <= 'זּ') || ('טּ' <= c && c <= 'לּ') || c == 'מּ' || ('נּ' <= c && c <= 'סּ') || ('ףּ' <= c && c <= 'פּ') || ('צּ' <= c && c <= '\ufbc1') || ('ﯓ' <= c && c <= 'ﴽ') || ('ﵐ' <= c && c <= 'ﶏ') || ('ﶒ' <= c && c <= 'ﷇ') || ('ﷰ' <= c && c <= '﷼') || ('ﹰ' <= c && c <= 'ﹴ') || ('ﹶ' <= c && c <= 'ﻼ') || (67584 <= c && c <= 67589) || c == 67592 || (67594 <= c && c <= 67637) || (67639 <= c && c <= 67640) || c == 67644 || (67647 <= c && c <= 67669) || (67671 <= c && c <= 67679) || (67840 <= c && c <= 67867) || (67872 <= c && c <= 67897) || c == 67903 || c == 68096 || (68112 <= c && c <= 68115) || (68117 <= c && c <= 68119) || (68121 <= c && c <= 68147) || (68160 <= c && c <= 68167) || (68176 <= c && c <= 68184) || (68192 <= c && c <= 68223) || (68352 <= c && c <= 68405) || (68416 <= c && c <= 68437) || (68440 <= c && c <= 68466) || (68472 <= c && c <= 68479)))
		{
			return true;
		}
		return false;
	...
	internal string DoBidiReverseOrder(string inputText, byte[] bidi, int trimCount)
	{
		m_indexLevels = new byte[inputText.Length];
		m_indexes = new int[inputText.Length];
		Array.Copy(bidi, trimCount, m_indexLevels, 0, m_indexLevels.Length);
		SetDefaultIndexLevel();
		DoOrder(0, m_indexLevels.Length - 1);
		string text = DoMirrorShaping(inputText);
		StringBuilder stringBuilder = new StringBuilder();
