...
internal class DataEncryption : ICipher
{
	...
	internal virtual int[] Keys => m_keys;
	...
	public virtual void Initialize(bool isEncryption, ICipherParam parameters)
	{
		...
		m_keys = GenerateWorkingKey(isEncryption, ((KeyParameter)parameters).Keys);
	}
	...
	public virtual int ProcessBlock(byte[] inputBytes, int inOffset, byte[] outputBytes, int outOffset)
	{
		...
		EncryptData(m_keys, inputBytes, inOffset, outputBytes, outOffset);
		return 8;
	}
	...
	protected static int[] GenerateWorkingKey(bool isEncrypt, byte[] bytes)
	{
		...
		{
			int num = Pc1[i];
			array2[i] = (bytes[num >>> 3] & ByteBit[num & 7]) != 0;
		}
		for (int j = 0; j < 16; j++)
		{
			int num2 = ((!isEncrypt) ? (15 - j << 1) : (j << 1));
			int num3 = num2 + 1;
	...
	internal static void EncryptData(int[] keys, byte[] inputBytes, int inOffset, byte[] outBytes, int outOffset)
	{
		uint num = Asn1Constants.BeToUInt32(inputBytes, inOffset);
		uint num2 = Asn1Constants.BeToUInt32(inputBytes, inOffset + 4);
		...
		for (int i = 0; i < 8; i++)
		{
			...
			num ^= num4;
			num3 = (num << 28) | (num >> 4);
			num3 ^= (uint)keys[i * 4 + 2];
			num4 = Sp7[num3 & 0x3F];
			num4 |= Sp5[(num3 >> 8) & 0x3F];
			num4 |= Sp3[(num3 >> 16) & 0x3F];
			num4 |= Sp1[(num3 >> 24) & 0x3F];
			num3 = num ^ (uint)keys[i * 4 + 3];
			num4 |= Sp8[num3 & 0x3F];
			num4 |= Sp6[(num3 >> 8) & 0x3F];
			num4 |= Sp4[(num3 >> 16) & 0x3F];
			num4 |= Sp2[(num3 >> 24) & 0x3F];
		...
		num ^= num3;
		num2 ^= num3 << 4;
		Asn1Constants.UInt32ToBe(num2, outBytes, outOffset);
		Asn1Constants.UInt32ToBe(num, outBytes, outOffset + 4);
	}
}
