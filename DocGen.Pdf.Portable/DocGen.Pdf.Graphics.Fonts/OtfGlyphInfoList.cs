...
internal class OtfGlyphInfoList
{
	...
	private bool HasAdvance()
	{
		...
		if (Glyphs != null && Glyphs.Count > 0)
		{
			foreach (OtfGlyphInfo glyph in Glyphs)
			{
				if (glyph.leadingY != 0 || glyph.leadingX != 0)
				{
					result = true;
					break;
				}
	...
	internal bool HasArabicScript()
	{
		...
		if (Glyphs != null && Glyphs.Count > 0)
		{
			foreach (OtfGlyphInfo glyph in Glyphs)
			{
				if (glyph.Characters != null && glyph.Characters.Length != 0)
				{
					ScriptTags language = languageUtil.GetLanguage(glyph.Characters[0]);
					if (!ScriptTags.Common.Equals(language) && !ScriptTags.Unknown.Equals(language) && !ScriptTags.Inherited.Equals(language) && language != ScriptTags.Arabic && language != ScriptTags.Latin)
					{
						result = false;
						break;
					}
