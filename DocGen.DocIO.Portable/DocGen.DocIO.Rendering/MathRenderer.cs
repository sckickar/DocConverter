...
internal class MathRenderer
{
	...
	internal DrawingContext DrawingContext => m_drawingContext;
	...
	internal void Draw(LayoutedLineWidget lineWidget)
	{
		if (!lineWidget.Skip)
		{
			IPen pen = WordDocument.RenderHelper.GetPen(lineWidget.Color, lineWidget.Width);
			DrawingContext.Graphics.DrawLine(pen, lineWidget.Point1, lineWidget.Point2);
		}
	}
	...
	private void Draw(LayoutedStringWidget stringWidget, WCharacterFormat characterFormat, float scalingFactor)
	{
		if (scalingFactor != 1f)
		{
			...
			WordDocument.RenderHelper.ApplyScale(matrix, scalingFactor, 1f);
			DrawingContext.Graphics.Transform = matrix;
		}
		IBrush solidBrush = WordDocument.RenderHelper.GetSolidBrush(DrawingContext.GetTextColor(characterFormat));
		StringFormat stringFormt = DrawingContext.StringFormt;
		DrawingContext.Graphics.DrawString(stringWidget.Text, stringWidget.Font, solidBrush, stringWidget.Bounds, stringFormt, FontScriptType.English);
		if (scalingFactor != 1f)
		{
			DrawingContext.ResetTransform();
		}
