...
internal sealed class Deflate
{
	...
	private static readonly Config[] config_table = new Config[10]
	{
	...
	{
		Buffer.BlockCopy(p, start, pending_buf, pending, len);
		pending += len;
	}
	...
	internal void fill_window()
	{
		do
		{
			...
			{
				Buffer.BlockCopy(window, w_size, window, 0, w_size);
				match_start -= w_size;
				strstart -= w_size;
				block_start -= w_size;
				num2 = hash_size;
	...
	internal int deflateSetDictionary(ZStream strm, byte[] dictionary, int dictLength)
	{
		...
		{
			num = w_size - 262;
			srcOffset = dictLength - num;
		}
		Buffer.BlockCopy(dictionary, srcOffset, window, 0, num);
		strstart = num;
		block_start = num;
		ins_h = window[0] & 0xFF;
