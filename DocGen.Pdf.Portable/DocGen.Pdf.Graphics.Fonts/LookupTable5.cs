	...
	internal override bool ReplaceGlyph(OtfGlyphInfoList glyphList)
	{
		...
		else
		{
			...
			foreach (GPOSRecordsCollection record2 in m_records)
			{
				...
				record2.Ligatures.TryGetValue(glyphInfoIndex.GlyphInfo.Index, out IList<GPOSValueRecord[]> value2);
				if (value2 == null)
				{
					continue;
				}
				...
				{
					if (value2[num][index] != null)
					{
						GPOSValueRecord gPOSValueRecord = value2[num][index];
						GPOSValueRecord record = value.Record;
						glyphList.Glyphs[glyphList.Index] = new OtfGlyphInfo(glyphList.Glyphs[glyphList.Index], gPOSValueRecord.XPlacement - record.XPlacement, gPOSValueRecord.YPlacement - record.YPlacement);
