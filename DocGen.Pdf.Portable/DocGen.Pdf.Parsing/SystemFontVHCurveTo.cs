...
internal class SystemFontVHCurveTo : SystemFontOperator
{
	public override void Execute(SystemFontBuildChar interpreter)
	{
		...
		if (operands.Count % 8 != 0 && operands.Count % 8 != 1)
		{
			...
			SystemFontOperator.CurveTo(interpreter, 0, firstAsInt, firstAsInt2, firstAsInt3, firstAsInt4, dyc);
			while (operands.Count / 8 > 0)
			{
				...
				SystemFontOperator.CurveTo(interpreter, firstAsInt5, 0, firstAsInt6, firstAsInt7, 0, firstAsInt8);
				SystemFontOperator.CurveTo(interpreter, 0, firstAsInt9, firstAsInt10, firstAsInt11, firstAsInt12, dyc);
			}
			operands.Clear();
		}
		else
		{
			while (operands.Count / 8 > 0)
			{
				...
				SystemFontOperator.CurveTo(interpreter, 0, firstAsInt13, firstAsInt14, firstAsInt15, firstAsInt16, 0);
				SystemFontOperator.CurveTo(interpreter, firstAsInt17, 0, firstAsInt18, firstAsInt19, dxc, firstAsInt20);
			}
			operands.Clear();
		}
	}
