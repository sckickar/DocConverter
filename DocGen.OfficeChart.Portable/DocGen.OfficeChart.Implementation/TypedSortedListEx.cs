...
internal class TypedSortedListEx<TKey, TValue> : IEnumerable<KeyValuePair<TKey, TValue>>, IEnumerable, IDictionary<TKey, TValue>, ICollection<KeyValuePair<TKey, TValue>>, IDictionary, ICollection where TKey : IComparable
{
	private class TypedSortedListExEnumerator : IEnumerator<KeyValuePair<TKey, TValue>>, IEnumerator, IDisposable, ICloneable
	{
	...
	private class ValueList : IList<TValue>, ICollection<TValue>, IEnumerable<TValue>, IEnumerable
	{
		...
		public virtual void UpdateValues()
		{
			...
			Sort(array, vals, m_list.m_comparer);
		}

		private static void Sort(TKey[] keys, TValue[] values, IComparer<TKey> comparer)
		{
	...
	public virtual TValue GetByName(string name)
	{
		...
		string[] array = new string[m_arrKeys.Length];
		m_arrKeys.CopyTo(array, 0);
		int num = Array.IndexOf(array, name);
		if (num < 0 || num >= m_arrKeys.Length)
		{
			throw new ArgumentOutOfRangeException("index");
		}
		return m_dicValues[m_arrKeys[num]];
	}
