...
internal class GPOSTableFormat : LookupTable
{
			...
			if (value != null)
			{
				...
				if (glyphInfoIndex.GlyphInfo != null)
				{
					...
					if (value2 != null)
					{
						...
						glyphList.Set(glyphList.Index, new OtfGlyphInfo(otfGlyphInfo, 0, 0));
	...
	private bool ReplaceFormat2Glyphs(OtfGlyphInfoList line)
	{
		...
		m_gposTableFormatRecord.TryGetValue(value, out var value2);
		if (value2 == null)
		{
			return false;
		}
		...
		_ = value2[value3];
		line.Set(line.Index, new OtfGlyphInfo(otfGlyphInfo, 0, 0));
	...
	private void ReadFormat1(int offset)
	{
		...
		for (int i = 0; i < num; i++)
		{
			...
			for (int j = 0; j < num2; j++)
			{
				...
				dictionary.Add(key, gPOSTableFormatRecord);
			}
		}
	}
