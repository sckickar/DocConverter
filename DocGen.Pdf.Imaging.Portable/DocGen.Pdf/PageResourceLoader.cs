...
internal sealed class PageResourceLoader
{
	...
	public PdfPageResourcesHelper GetPageResources(PdfPageBase page)
	{
		PdfPageResourcesHelper pageResources = new PdfPageResourcesHelper();
				...
				if (pdfDictionary != null)
				{
					foreach (KeyValuePair<PdfName, IPdfPrimitive> item in pdfDictionary.Items)
					{
						if (item.Value is PdfReferenceHolder)
						{
							...
							{
								dictionary.Add(item.Key.Value, new FontStructureHelperNet((item.Value as PdfReferenceHolder).Object, (item.Value as PdfReferenceHolder).Reference.ToString()));
							}
							else
							{
								dictionary.Add(item.Key.Value, new FontStructureHelperNet((item.Value as PdfReferenceHolder).Object));
							}
						}
						else if (item.Value is PdfDictionary fontDictionary)
						{
							dictionary.Add(item.Key.Value, new FontStructureHelperNet(fontDictionary));
						}
						else
						{
							dictionary.Add(item.Key.Value, new FontStructureHelperNet(item.Value, (item.Value as PdfReferenceHolder).Reference.ToString()));
						}
					}
				}
			}
			...
			if (pdfPrimitive2 != null)
			{
				...
				if (pdfPrimitive != null && pdfPrimitive is PdfDictionary pdfDictionary2)
				{
					foreach (KeyValuePair<PdfName, IPdfPrimitive> item2 in pdfDictionary2.Items)
					{
						...
						dictionary.Add(item2.Key.Value, new FontStructureHelperNet(item2.Value, (item2.Value as PdfReferenceHolder).Reference.ToString()));
					}
				}
			}
		}
			...
			if (pdfPrimitive is PdfDictionary)
			{
				foreach (KeyValuePair<PdfName, IPdfPrimitive> item in ((PdfDictionary)pdfPrimitive).Items)
				{
					if (item.Value is PdfReferenceHolder)
					{
						...
						{
							ImageStructureNet value = new ImageStructureNet(pdfDictionary, new PdfMatrix());
							if (commonMatrix.ContainsKey(item.Key.Value))
							{
								value = new ImageStructureNet(pdfDictionary, commonMatrix[item.Key.Value]);
							}
							dictionary.Add(item.Key.Value, value);
							m_hasImages = true;
						}
							...
							if (pdfDictionary.ContainsKey("Resources") && pdfDictionary["Resources"] is PdfDictionary)
							{
								foreach (KeyValuePair<PdfName, IPdfPrimitive> item2 in (pdfDictionary["Resources"] as PdfDictionary).Items)
								{
									...
									foreach (KeyValuePair<PdfName, IPdfPrimitive> item3 in items)
									{
										...
										{
											PdfMatrix value2 = new PdfMatrix(new PdfReader(new MemoryStream((pdfDictionary as PdfStream).GetDecompressedData()))
											{
												Position = 0L
											}, item3.Key.Value, page.Size);
											commonMatrix.Add(item3.Key.Value, value2);
										}
									}
								}
							}
	...
	internal PdfPageResourcesHelper UpdatePageResources(PdfPageResourcesHelper pageResources, Dictionary<string, object> objects)
	{
