...
internal class TtfReader
{
	...
	static TtfReader()
	{
		...
		glyphFonts = new HashSet<string>(new string[6] { "gautami", "latha", "shruti", "mangal", "tunga", "vrinda" }, System.StringComparer.OrdinalIgnoreCase);
	...
	internal TtfReader(BinaryReader reader, bool isFloatingPointer)
	{
		...
		m_reader = new BigEndianReader(reader);
		m_useFloatingFactorForMeasure = isFloatingPointer;
		Initialize();
	}
	...
	public TtfGlyphInfo GetGlyph(char charCode)
	{
		...
		else if (m_macintosh != null && (m_metrics.IsSymbol || m_isMacTTF || (m_microsoft != null && m_microsoft.Count == 0) || m_microsoft == null))
		{
			...
			if (obj == null && ((PdfDocument.ConformanceLevel != 0 && PdfDocument.ConformanceLevel != PdfConformanceLevel.Pdf_X1A2001) || PdfDocument.FontEmbeddingEnabled))
			{
	...
	{
		supportedScriptTags.Add(ScriptTags.Arabic);
	...
	private void ReadMetrics()
	{
		...
		m_EmHeight = headTable.UnitsPerEm;
		...
		m_CellAscent = os2Table.UsWinAscent;
		m_fsSelection = os2Table.FsSelection;
		m_stypoAscent = os2Table.STypoAscender;
		TtfPostTable postTable = ReadPostTable();
		TtfTableInfo table = GetTable("maxp");
		m_reader.Seek(table.Offset);
		m_reader.ReadInt32();
		int num = m_reader.ReadInt16();
		if (num > 0)
		{
			m_width = ReadWidthTable(num, horizontalHeadTable.NumberOfHMetrics, headTable.UnitsPerEm);
		}
		else
		{
	...
	private void InitializeMetrics(TtfNameTable nameTable, TtfHeadTable headTable, TtfHorizontalHeaderTable horizontalHeadTable, TtfOS2Table os2Table, TtfPostTable postTable, TtfCmapSubTable[] cmapTables)
	{
		...
		m_metrics.WinAscent = (float)os2Table.STypoAscender * num;
		m_metrics.MacAscent = (float)horizontalHeadTable.Ascender * num;
		m_metrics.CapHeight = ((os2Table.SCapHeight != 0) ? ((float)os2Table.SCapHeight) : (0.7f * (float)(int)headTable.UnitsPerEm * num));
		m_metrics.WinDescent = (float)os2Table.STypoDescender * num;
		m_metrics.MacDescent = (float)horizontalHeadTable.Descender * num;
		m_metrics.Leading = (float)(os2Table.STypoAscender - os2Table.STypoDescender + os2Table.STypoLineGap) * num;
		m_metrics.LineGap = (int)Math.Ceiling((float)horizontalHeadTable.LineGap * num);
		int x = (int)((float)headTable.XMin * num);
		int num2 = 0;
		int num3 = 0;
		num2 = ((m_metrics.MacAscent == 0f) ? headTable.YMax : ((int)Math.Ceiling(m_metrics.MacAscent + (float)m_metrics.LineGap)));
		int x2 = (int)((float)headTable.XMax * num);
		num3 = ((m_metrics.MacAscent == 0f) ? headTable.YMin : ((int)m_metrics.MacDescent));
		m_metrics.FontBox = new RECT(x, num2, x2, num3);
		m_metrics.StemV = 80f;
	...
	private float[] ReadWidthTable(int glyphCount, int unitsPerEm)
	{
		TtfTableInfo table = GetTable("hmtx");
		m_reader.Seek(table.Offset);
		float[] array = new float[glyphCount];
		for (int i = 0; i < glyphCount; i++)
		{
			...
			if (unitsPerEm > 0)
			{
				float num = (float)(ttfLongHorMertric.AdvanceWidth * 1000) / (float)unitsPerEm;
				if (!UseFloatingFactorForMeasure)
				{
					num = (int)num;
				}
				array[i] = num;
	...
	private float[] ReadWidthTable(int glyphCount, int gCount, int unitsPerEm)
	{
		TtfTableInfo table = GetTable("hmtx");
		m_reader.Seek(table.Offset);
		float[] array = new float[glyphCount];
		...
		float num2 = array[gCount - 1];
		if (gCount <= glyphCount)
		{
			for (int j = gCount; j < glyphCount; j++)
			{
				array[j] = num2;
			}
		}
	...
	private float[] UpdateWidth()
	{
		...
		if (m_metrics.IsSymbol || conformanceEnabled)
		{
			for (int i = 0; i < num; i++)
			{
				TtfGlyphInfo glyph = GetGlyph((char)i);
				array[i] = (glyph.Empty ? 0f : glyph.Width);
			}
		}
		else
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				array[j] = (glyph2.Empty ? 0f : glyph2.Width);
			}
		}
		return array;
	}
	...
	private void ProcessCompositeGlyph(Dictionary<int, int> glyphChars, int glyph, TtfLocaTable locaTable)
	{
		...
		while (true)
		{
			...
			int num4 = m_reader.ReadUInt16();
			if (!glyphChars.ContainsKey(num4))
			{
				glyphChars.Add(num4, 0);
				long position = m_reader.BaseStream.Position;
				ProcessCompositeGlyph(glyphChars, num4, locaTable);
				m_reader.Seek(position);
			}
			if ((num3 & 0x20u) != 0)
			{
	...
	{
		Dictionary<int, int> allGlyphChars = GetAllGlyphChars();
		TtfLocaTable locaTable = ReadLocaTable(m_bIsLocaShort);
		UpdateGlyphChars(allGlyphChars, locaTable);
		int[] newLocaTable = null;
		byte[] newGlyphTable = null;
		byte[] newLocaTableOut = null;
		uint glyphTableSize = GenerateGlyphTable(allGlyphChars, locaTable, out newLocaTable, out newGlyphTable);
		int locaTableSize = UpdateLocaTable(newLocaTable, m_bIsLocaShort, out newLocaTableOut);
		return GetFontProgram(newLocaTableOut, newGlyphTable, glyphTableSize, (uint)locaTableSize);
	}

	internal Dictionary<int, int> GetAllGlyphChars()
	{
		Dictionary<int, int> dictionary = new Dictionary<int, int>();
		List<TtfGlyphInfo> allGlyphs = GetAllGlyphs();
		for (int i = 0; i < allGlyphs.Count; i++)
		{
			TtfGlyphInfo ttfGlyphInfo = allGlyphs[i];
			if (!ttfGlyphInfo.Empty)
			{
				dictionary[ttfGlyphInfo.Index] = ttfGlyphInfo.CharCode;
			}
		}
		return dictionary;
	}
