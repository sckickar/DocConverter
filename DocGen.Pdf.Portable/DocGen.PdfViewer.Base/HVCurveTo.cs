...
internal class HVCurveTo : Operator
{
	public override void Execute(CharacterBuilder interpreter)
	{
		...
		if (operands.Count % 8 != 0 && operands.Count % 8 != 1)
		{
			...
			Operator.CurveTo(interpreter, firstAsInt, 0, firstAsInt2, firstAsInt3, dxc, firstAsInt4);
			while (operands.Count / 8 > 0)
			{
				...
				Operator.CurveTo(interpreter, 0, firstAsInt5, firstAsInt6, firstAsInt7, firstAsInt8, 0);
				Operator.CurveTo(interpreter, firstAsInt9, 0, firstAsInt10, firstAsInt11, dxc, firstAsInt12);
			}
			operands.Clear();
		}
		else
		{
			while (operands.Count / 8 > 0)
			{
				...
				Operator.CurveTo(interpreter, firstAsInt13, 0, firstAsInt14, firstAsInt15, 0, firstAsInt16);
				Operator.CurveTo(interpreter, 0, firstAsInt17, firstAsInt18, firstAsInt19, firstAsInt20, dyc);
			}
			operands.Clear();
		}
	}
