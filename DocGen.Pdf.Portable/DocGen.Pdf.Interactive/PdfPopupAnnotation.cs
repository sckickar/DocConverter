...
public class PdfPopupAnnotation : PdfAnnotation
{
	...
	internal PdfPopupAnnotation(RectangleF bounds, bool isPopup)
		: base(bounds)
	{
		base.Dictionary.SetProperty("Subtype", new PdfName("Popup"));
	...
	{
		base.Initialize();
		base.Dictionary.SetProperty("Subtype", new PdfName("Text"));
	}
	...
	private void SaveAndFlatten(bool isExternalFlattenPopUps)
	{
		...
		else if (!m_isCustomIconEnabled && !base.Dictionary.ContainsKey("Name"))
		{
		...
		if (base.Flatten)
		{
			...
			if (m_appearance != null && m_appearance.Normal != null)
			{
				if (base.Page != null)
				{
					if (Opacity < 1f)
					{
						...
						base.Page.Graphics.DrawPdfTemplate(m_appearance.Normal, Bounds.Location);
				...
				else if (base.LoadedPage != null)
				{
					if (Opacity < 1f)
					{
						...
						base.LoadedPage.Graphics.DrawPdfTemplate(m_appearance.Normal, Bounds.Location);
			...
			if (base.FlattenPopUps || isExternalFlattenPopUps)
			{
				...
				{
					RemoveAnnoationFromPage(m_loadedPage, this);
					IPdfPrimitive @object = base.LoadedPage.CrossTable.GetObject(base.Dictionary);
					int num = base.LoadedPage.CrossTable.PdfObjects.IndexOf(@object);
					if (num != -1)
					{
						base.LoadedPage.CrossTable.PdfObjects.Remove(num);
					}
					base.LoadedPage.TerminalAnnotation.Remove(base.Dictionary);
				}
			}
		}
		else
	...
	private void CreateApearance()
	{
		if (m_appearance == null)
		{
			PdfGraphics graphics = Appearance.Normal.Graphics;
			PdfGraphics graphics2 = Appearance.MouseHover.Graphics;
			...
			if (m_isCustomIconEnabled && m_iconName == "Checkmark")
			{
